...
For live parity measurement, run `scripts/agent-parity-report.sh [database-path] <session-id>`. It reports each top-level runtime run separately, including provider calls, tool calls, failures, elapsed time, uncached input, cache writes, cache reads, output, and logical total. Delegated runs are listed separately so a compact parent-session UI cannot conceal child work. Fair external comparisons must use the same repository snapshot, task, model, reasoning level, permission mode, skill/MCP catalog, and warm/cold-cache condition.
