...
Foreground Bash keeps a recoverable full-output spool only when the model-facing preview is truncated. Canonical runs place that runtime spool beside the session database rather than under the project workspace; standalone direct-tool contexts without a database retain the legacy `.mitsuro/tool-output` fallback. Read-only commands in a normal agent run therefore do not create diagnostic directories in user source trees.
...
- Anthropic-compatible caching emits the stable base and project sections before uncached session context, with explicit cacheable system blocks only for providers that advertise support. `MITSURO_CACHE_RETENTION=long` requests Anthropic's one-hour cache lifetime.
