...
| --- | --- | --- | --- |
| **OpenAI** | API keys query `/v1/models`; ChatGPT OAuth queries the account-scoped Codex catalog. Rows are merged by exact provider/auth/API-format key, so the same wire slug can remain available through both credential identities without one overwriting the other. | GPT-5.6 Sol/Terra/Luna, GPT-5.5/5.5 Pro, GPT-5.4/5.4 Pro/5.4 Mini/5.4 Nano, Chat Latest, and GPT-5.3 Codex/Spark. | Successful live API/OAuth discovery is authoritative because each identity can expose different entitlements, context limits, reasoning presets, and Fast eligibility. |
...
| **Grok** | Queries the authenticated Grok CLI subscription proxy's `/models` endpoint. | Grok Build, Composer 2.5, and Grok 4.6 when listed. | Classic `grok-build` / Composer rows are **output-only** (reasoning may stream, no effort Tab). **Grok 4.6** exposes graded `reasoning_effort` (**low / medium / high / extra-high (`xhigh`)**, default high, always-on). Retired Grok 4.5 rows are omitted from curated and live lists. |
...
- **Grok 4.6** uses OpenAI-style graded effort: `reasoning.effort` = low/medium/high/`xhigh` on the Responses transport. 4.3-style IDs remain optional and default low when present. Grok 4.5 is retired from product catalogs.
