...
| **Grok** | Queries the authenticated Grok CLI subscription proxy's `/models` endpoint. | Grok Build, Composer 2.5, and Grok 4.5 when listed. | Classic `grok-build` / Composer rows are **output-only** (reasoning may stream, no effort Tab). Public-style **Grok 4.5** exposes graded `reasoning_effort` (**low / medium / high**, default high, always-on). |
