...
Task scope: convert Mitsuro's existing RetroArch/libretro TUI plugin into a Game Boy Color branded plugin that can ship/prefer a bundled Gambatte/SameBoy libretro core and keep emulator runtime state host-owned across plugin shell reloads. No source files were modified while gathering this context.

## Current architecture

### Built-in TUI plugin registry

- `crates/mitsuro-cli/src/tui/plugins/mod.rs:18-25` conditionally exposes `gamepad`, `libretro`, and `retroarch` only on Unix. Non-Unix gets a no-op `GamepadHandler` but not libretro/retroarch.
...
- `/plugins reload <plugin-id>` is only for installable plugins. It calls `PluginManager::reload_plugin`, and if the active plugin ID matches, it rebuilds the shell with `get_plugin_by_id()` and `set_plugin()` (`handlers/commands/plugins/subcommands.rs:111-129`). Because built-in `retroarch` is not in the installed plugin lockfile, `/plugins reload retroarch` currently fails in `PluginManager::reload_plugin()` (`mitsuro-core/src/plugins/manager/mod.rs:107-113`).
...
- `crates/mitsuro-cli/src/tui/plugins/libretro.rs` defines raw libretro FFI. `LibRetroCore` owns a `libloading::Library` and function pointers (`libretro.rs:164-195`), loads a dynamic library (`libretro.rs:197-207`), resolves required symbols (`libretro.rs:217-267`), and validates API version 1 (`libretro.rs:270-276`). Current loading is Unix-only (`libretro.rs:202-283`).
...
- Environment callbacks use static paths under `~/.config/mitsuro/retroarch/{system,saves}` (`retroarch.rs:335-348`) and `MitsuroDirs` uses `~/.config/mitsuro/retroarch/{system,saves,states,roms}` (`retroarch.rs:468-498`). This is inconsistent with core path utilities/docs that use `~/.mitsuro` for plugins and app state.
...
- Branding is currently hard-coded as RetroArch:
  ...
  - Logs and env vars use `MITSURO_RETROARCH_CORE` / `MITSURO_RETROARCH_ROM` (`retroarch.rs:1583-1608`).
...
- `PluginManager` manages signed installable plugins under `~/.mitsuro/plugins` with `installed`, `active`, `state`, `index`, `trust`, and `plugins.lock` roots (`mitsuro-core/src/plugins/manager/mod.rs:21-67`; docs `docs/extensions/mcp-and-plugins.md:102-118`).
- Manifest v1 describes a single `entry_component` artifact plus render capabilities and permissions (`mitsuro-core/src/plugins/types.rs:44-89`; docs `mcp-and-plugins.md:82-90`).
...
- Current `ManagedPlugin` is only a lightweight placeholder, not a WASM/runtime execution host. It renders static text or demo frames and handles `r` locally by resetting animation (`crates/mitsuro-cli/src/tui/plugins/managed.rs`).
...
- Libretro docs confirm cores use frontend-provided system/save directories and `RETRO_MEMORY_SAVE_RAM` for cartridge battery-backed save RAM; Mitsuro's existing implementation already provides `GET_SYSTEM_DIRECTORY`, `GET_SAVE_DIRECTORY`, save-state serialization, and SRAM memory handling.
...
4. **Prefer a bundled SameBoy/Gambatte core via an explicit resolver.** Replace `scan_cores()` with a GBC core resolver:
   - Candidate order: env override (`MITSURO_GAMEBOY_CORE` and possibly legacy `MITSURO_RETROARCH_CORE`), extracted bundled SameBoy, extracted bundled Gambatte (if approved), user-installed cores under `~/.mitsuro/gameboy-color/cores`, system `/usr/lib/libretro/{sameboy,gambatte}_libretro.so`.
   ...
   - If bundling as release/package assets instead, wire package/install scripts to place cores under `~/.mitsuro/gameboy-color/cores` or an app resource dir and teach the resolver where to find them.
...
7. **Use centralized app paths.** Prefer `crate::paths::config_dir().join("gameboy-color")` or `mitsuro_core::paths::config_dir()` over the current `~/.config/mitsuro/retroarch`. Keep `system`, `saves`, `states`, `roms`, `cores` directories under that base. Consider migration or reading legacy save dirs if existing users matter.

## Files/functions likely to change

- `crates/mitsuro-cli/src/tui/plugins/mod.rs`
...
- `crates/mitsuro-cli/src/tui/plugins/retroarch.rs` and/or new `crates/mitsuro-cli/src/tui/plugins/gameboy.rs`
...
- `crates/mitsuro-cli/src/tui/components/plugin_window.rs`
...
- `cargo test -p mitsuro` for TUI plugin unit tests.
- `cargo check -p mitsuro` after refactor.
- If touching core plugin manager: `cargo test -p mitsuro-core plugins::manager` and `cargo check --workspace`.
...
- **State path migration.** Existing saves are under `~/.config/mitsuro/retroarch`; new branded paths should probably live under `~/.mitsuro/gameboy-color`. Decide whether to migrate/read legacy saves/states.
...
- Relevant paths checked:
  - `crates/mitsuro-cli/Cargo.toml` is staged-modified (version `0.6.2 -> 0.7.0`). If adding deps/features, preserve this user change.
  - `crates/mitsuro-cli/src/tui/app_builder.rs` is staged-modified in model registry code, not plugin setup. Early plugin-manager initialization lines are unchanged, but avoid overwriting this file unless necessary.
  - No dirty changes were reported under `crates/mitsuro-cli/src/tui/plugins/`, `crates/mitsuro-cli/src/tui/components/plugin_window.rs`, `crates/mitsuro-cli/src/tui/handlers/commands/plugins/`, `crates/mitsuro-core/src/plugins/`, or `docs/extensions/mcp-and-plugins.md` at the time of inspection.
