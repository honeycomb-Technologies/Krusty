...
private final class MitsuroPerformanceSignposts {
  static let shared = MitsuroPerformanceSignposts()

  private let signposter = OSSignposter(
    subsystem: Bundle.main.bundleIdentifier ?? "io.mitsuro.mobile",
...
private final class MitsuroMetricKitCollector: NSObject, MXMetricManagerSubscriber {
  static let shared = MitsuroMetricKitCollector()

  private let queue = DispatchQueue(label: "io.mitsuro.mobile.metrics", qos: .utility)
    ...
    queue.sync {
      ...
      for file in files {
        guard
          ...
          // Canonical corrupt files can be retired, but the compatibility
          // reader must not mutate the legacy queue needed by an older binary.
          if file.deletingLastPathComponent() == payloadDirectory() {
            try? FileManager.default.removeItem(at: file)
          }
          continue
        }
        guard seenIDs.insert(stored.id).inserted else { continue }
        ...
        if records.count == maxStoredPayloads {
          break
        }
      }
      return records
  ...
  private func persist(_ payloads: [(String, MetricKitSummary)]) {
    queue.async {
      self.migrateLegacyPayloadsIfNeeded()
  ...
  private func enforceBounds() {
    // Legacy payloads remain owned by the compatibility window. Bound only
    // the canonical queue so a downgrade can still recover unacknowledged data.
    var files = payloadFiles(in: payloadDirectory())
  ...
  private func payloadFiles() -> [URL] {
    payloadFiles(in: payloadDirectory()) + payloadFiles(in: legacyMetricKitPayloadDirectory())
  }

  private func payloadFiles(in directory: URL) -> [URL] {
  ...
  private func migrateLegacyPayloadsIfNeeded() {
    ...
    let legacyDirectory = legacyMetricKitPayloadDirectory()
    let canonicalDirectory = payloadDirectory()
    guard legacyDirectory != canonicalDirectory,
    ...
    do {
      ...
      for source in payloadFiles(in: legacyDirectory) {
        let sourceData = try Data(contentsOf: source)
        var destination = canonicalDirectory.appendingPathComponent(source.lastPathComponent)
        if fileManager.fileExists(atPath: destination.path) {
          let destinationData = try? Data(contentsOf: destination)
          if sourceData == destinationData {
            continue
          }
          destination = canonicalDirectory.appendingPathComponent("legacy-\(source.lastPathComponent)")
          if fileManager.fileExists(atPath: destination.path) {
            let importedData = try? Data(contentsOf: destination)
            if sourceData == importedData {
              continue
            }
            destination = canonicalDirectory.appendingPathComponent(
              "legacy-\(UUID().uuidString.lowercased())-\(source.lastPathComponent)"
            )
          }
        }
      ...
      // Reads continue to include the prior directory, and imports never
      // remove it, so a failed copy cannot break downgrade recovery. A later
      // process launch retries the bounded, ID-deduplicated migration.
  ...
  private func payloadDirectory() -> URL {
    ...
    return base.appendingPathComponent("MitsuroDiagnostics/MetricKit", isDirectory: true)
  }
}
...
public class MitsuroDiagnosticsModule: Module {
  public func definition() -> ModuleDefinition {
    mitsuroDiagnosticsDefinition(moduleName: "MitsuroDiagnostics")
  }
}
