...
function ChatStreamActivity(props: ChatStreamProps, env: LiveActivityEnvironment): LiveActivityLayout {
  'widget';

  const {
    chatTitle, model, status, currentText, currentTool,
    elapsedSeconds, tokenCount, progress, toolApprovalId, toolApprovalName,
  } = props;

  const statusColor = status === 'streaming' ? '#ff6b35'
    : status === 'thinking' ? '#fbbf24'
    : status === 'tool_call' ? '#60a5fa'
    : status === 'awaiting_approval' ? '#ef4444'
    : '#4ade80';

  const statusLabel = status === 'streaming' ? 'Streaming'
    : status === 'thinking' ? 'Thinking'
    : status === 'tool_call' ? currentTool
    : status === 'awaiting_approval' ? 'Approval Required'
    : 'Complete';

  return {
    // Dynamic Island — compact leading (left of camera)
    compactLeading: (
      <HStack spacing={4}>
        <Image
    ...
    // Dynamic Island — compact trailing (right of camera)
    compactTrailing: (
      <HStack spacing={4}>
    ...
    // Dynamic Island — minimal (when 2+ live activities)
    minimal: (
      <Image
    ...
    expandedLeading: (
      <Image
        systemName={
          status === 'thinking' ? 'brain'
          : status === 'tool_call' ? 'wrench.fill'
          : status === 'awaiting_approval' ? 'exclamationmark.shield.fill'
    ...
    expandedTrailing: (
      <VStack alignment="trailing" spacing={2}>
        <Text modifiers={[font({ size: 11, design: 'monospaced' }), foregroundStyle(statusColor)]}>
    ...
    expandedBottom: (
      <VStack spacing={6}>
        ...
        <ProgressView value={progress} modifiers={[foregroundStyle(statusColor)]} />
    ...
    // Lock screen banner (the most spacious layout)
    banner: (
      <VStack alignment="leading" spacing={8} modifiers={[padding({ all: 16 })]}>
        <HStack>
          ...
          <Spacer />
          <VStack alignment="trailing" spacing={2}>
            <Text modifiers={[font({ size: 13, weight: 'medium', design: 'monospaced' }), foregroundStyle(statusColor)]}>
              {formatElapsed(elapsedSeconds)}
            </Text>
