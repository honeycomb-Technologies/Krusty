...
import type { HiveCurrentRunSummary } from "@mitsuro/api";
...
interface HiveScheduleViewProps {
...
function scheduleSeriesKey(run: HiveCurrentRunSummary, wakeAt: string): string {
...
function buildScheduledItems(runs: HiveCurrentRunSummary[]): ScheduledRunItem[] {
  return runs
    .flatMap((run) => {
...
export function HiveScheduleView({
  state,
  onSelectRun,
  onOpenProject,
}: HiveScheduleViewProps) {
  ...
  const handleWakeNow = async () => {
    ...
    setDetailError(null);
    try {
      await client.resumeHiveSession(detailTarget.item.run.session_id);
      await state.refresh();
      setDetailTarget(null);
    } catch (error) {
      setDetailError(
