...
import { HiveApprovalList } from "./HiveApprovalList";
...
import type {
  HiveDaemonSummary,
  HiveDiagnosticsSummary,
  HiveHealthState,
  HiveKnowledgeHealthSummary,
} from "@mitsuro/api";

interface HiveStatusViewProps {
...
export function HiveStatusView({
  state,
  activeToolCallId,
  onSelectRun,
  onApproveTool,
...
function summarizeCadence(runs: HiveCurrentRunSummary[]) {
  if (!runs.length) {
    return {
...
function summarizeQueueHealth(
  ...
  diagnostics?: HiveDiagnosticsSummary | null,
) {
  if (diagnostics) {
...
function formatHealthState(state?: HiveHealthState | null): string {
  switch (state) {
    case "healthy":
      return "Healthy";
    case "attention":
...
function summarizePriorityProfile(runs: HiveCurrentRunSummary[]) {
  const counts = {
...
function summarizeKnowledgeHealth(
  health?: HiveKnowledgeHealthSummary | null,
) {
...
function summarizeDaemonHealth(
  daemon?: HiveDaemonSummary | null,
  healthState?: HiveHealthState | null,
) {
  if (!daemon) {
    return {
      value: "Pending",
