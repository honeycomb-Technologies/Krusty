...
import type { HiveAttentionResponse, HiveCurrentResponse } from "@mitsuro/api";
import { useConnection } from "../../../hooks/useConnection";
import type { HiveAttentionItem } from "../types";

interface AttentionSections {
  needsAction: HiveAttentionItem[];
  updates: HiveAttentionItem[];
}

function mapItem(
  item: HiveAttentionResponse["items"][number],
): HiveAttentionItem {
  return {
    id: item.id,
    kind: item.kind,
...
export function useHiveAttention(
  ...
  const refresh = useCallback(async () => {
    ...
    setError(null);
    try {
      const response = await client.getHiveAttention({
        threadSessionId,
      });
  ...
  const markRead = useCallback(
    async (itemId: string, read: boolean) => {
      ...
      try {
        await client.setHiveAttentionRead(itemId, read);
      } catch (markError) {
        setError(
  ...
  const clearItem = useCallback(
    async (itemId: string) => {
      ...
      try {
        await client.setHiveAttentionCleared(itemId, true);
      } catch (clearError) {
        setError(
