...
import { HiveKnowledgeScopeToggle } from "./HiveKnowledgeScopeToggle";
import { HiveMemoryView } from "./HiveMemoryView";
import { HiveTopNav } from "./HiveTopNav";
...
import type { HiveKnowledgeView } from "./types";

interface HiveReportsViewProps {
...
export function HiveReportsView({ workspaceDirectory }: HiveReportsViewProps) {
  const { theme } = useThemeContext();
  const { isDesktop } = useBreakpoint();
  const { client } = useConnection();
  const t = theme.colors;
  const [knowledgeView, setKnowledgeView] = useState<HiveKnowledgeView>("recent");
  const [query, setQuery] = useState("");
  const [isPromoting, setIsPromoting] = useState(false);
  const [promotionState, setPromotionState] = useState<PromotionState | null>(null);
  const reports = useHiveReports(knowledgeView === "recent", workspaceDirectory, query);
  const memories = useHiveMemories(knowledgeView === "memory", workspaceDirectory);
