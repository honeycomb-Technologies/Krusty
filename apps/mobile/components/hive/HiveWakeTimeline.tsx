...
import type { HiveRunWakeEvent } from "@mitsuro/api";

interface HiveWakeTimelineProps {
  wake: HiveRunWakeEvent[];
}

export function HiveWakeTimeline({ wake }: HiveWakeTimelineProps) {
      ...
      {wake.map((event) => (
        <GlassCard key={event.id} style={styles.card}>
          <View style={styles.row}>
            ...
            <HiveStatusBadge status={event.status} />
          </View>
