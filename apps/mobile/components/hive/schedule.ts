export type HiveSchedulePreset = "now" | "30m" | "2h" | "tomorrow" | "custom";

export interface HiveScheduleResolution {
  startAt: string | null;
  error: string | null;
}

interface HiveSchedulePresetOption {
  id: HiveSchedulePreset;
  label: string;
}

const ALL_PRESETS: HiveSchedulePresetOption[] = [
  { id: "now", label: "Now" },
  { id: "30m", label: "30m" },
  { id: "2h", label: "2h" },
...
export function resolveScheduleSelection(
  preset: HiveSchedulePreset,
  customValue?: string | null,
): HiveScheduleResolution {
  const now = new Date();
  switch (preset) {
    case "now":
