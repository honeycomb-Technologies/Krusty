...
export const HIVE_PRIORITY_OPTIONS: Array<{
  id: HiveRunPriority;
  label: string;
}> = [
  { id: "low", label: "Low" },
...
export function describePriority(priority: HiveRunPriority): string {
  switch (priority) {
    case "high":
...
export function formatPriorityLabel(priority: HiveRunPriority): string {
  switch (priority) {
    case "high":
