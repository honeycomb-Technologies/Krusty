...
impl ApnsService {
    ...
    pub fn accepts_registration(&self, bundle_id: &str, environment: &str) -> bool {
        self.config.bundle_id == bundle_id
            && environment
                == if self.config.sandbox {
                    "sandbox"
                } else {
    ...
    async fn send_to_device(
        ...
        let apns_payload = build_apns_json(payload, &self.config.bundle_id, event_type, play_sound);
        let encoded_payload =
...
fn apns_priority(event_type: ApnsEventType) -> &'static str {
    match event_type {
        ...
        ApnsEventType::Completion | ApnsEventType::MakoUpdate | ApnsEventType::Test => "5",
    }
}
...
fn apns_ttl_seconds(event_type: ApnsEventType) -> u64 {
    match event_type {
        ...
        ApnsEventType::MakoUpdate => 24 * 60 * 60,
...
fn build_apns_json(
    ...
    let mut aps = serde_json::json!({
        ...
        "interruption-level": match event_type {
            ApnsEventType::Completion | ApnsEventType::MakoUpdate => "passive",
            _ => "active",
        },
    });
    if play_sound {
...
mod tests {
    use super::{
        ...
        summarize_apns_failure_body, ApnsEventType, ApnsPayload, DEFAULT_APNS_BUNDLE_ID,
    };
    use krusty_core::storage::{
