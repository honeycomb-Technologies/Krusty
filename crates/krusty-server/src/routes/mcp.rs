...
use krusty_core::mcp::{McpConfigSource, McpServerInfo, McpServerStatus, McpToolApproval};
...
) -> Result<Json<krusty_core::mcp::McpOAuthStatus>, AppError> {
    state
        .mcp_manager
        .oauth_status(&name)
        .await
...
) -> Result<Json<krusty_core::mcp::McpOAuthStart>, AppError> {
    state
        .mcp_manager
...
fn tool_response(tool: krusty_core::mcp::McpToolDef) -> McpToolResponse {
    McpToolResponse {
        name: tool.name,
        title: tool.title,
