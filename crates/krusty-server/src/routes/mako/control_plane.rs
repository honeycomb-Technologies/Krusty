...
) -> Result<Json<Vec<MakoSchedule>>, AppError> {
...
pub(super) async fn create_schedule(
    ...
    ensure_owned_mako_session(&state, user.as_ref(), &session_id)?;
...
pub(super) async fn list_occurrences(
    ...
    Path((session_id, schedule_id)): Path<(String, String)>,
    Query(query): Query<ListQuery>,
) -> Result<Json<Vec<MakoScheduleOccurrence>>, AppError> {
    let controller = require_owned_controller(&state, user.as_ref(), &session_id)?;
...
pub(super) async fn list_runs(
    ...
    Path(session_id): Path<String>,
    Query(query): Query<ListQuery>,
) -> Result<Json<Vec<MakoRun>>, AppError> {
    let Some(controller) = owned_controller(&state, user.as_ref(), &session_id)? else {
        return Ok(Json(Vec::new()));
    };
    let store = MakoRunStore::new(Database::new(&state.db_path)?);
    Ok(Json(store.list_for_controller(
        &controller.id,
        list_limit(query.limit),
    )?))
...
pub(super) async fn list_attempts(
    ...
    user: Option<CurrentUser>,
    Path((session_id, run_id)): Path<(String, String)>,
) -> Result<Json<Vec<MakoRunAttempt>>, AppError> {
    let controller = require_owned_controller(&state, user.as_ref(), &session_id)?;
...
pub(super) async fn list_event_log(
    ...
    Path(session_id): Path<String>,
    Query(query): Query<ListQuery>,
) -> Result<Json<Vec<MakoControllerEvent>>, AppError> {
    let Some(controller) = owned_controller(&state, user.as_ref(), &session_id)? else {
        return Ok(Json(Vec::new()));
    };
    let store = MakoControllerEventStore::new(Database::new(&state.db_path)?);
...
fn owned_controller(
    ...
    user: Option<&CurrentUser>,
    session_id: &str,
) -> Result<Option<MakoController>, AppError> {
    ensure_owned_mako_session(state, user, session_id)?;
    Ok(MakoControllerStore::new(Database::new(&state.db_path)?).get_by_session(session_id)?)
}
...
fn owned_schedule(
    state: &AppState,
    controller: &MakoController,
    schedule_id: &str,
) -> Result<MakoSchedule, AppError> {
    let schedule = MakoScheduleStore::new(Database::new(&state.db_path)?)
