...
pub(super) async fn steer(
    ...
    let status = if sender.send(input.clone()).is_ok() {
        "accepted"
    } else {
        // A run can roll over between cloning its sender and delivery. Retry a
        // replacement sender once with the same durable ID; never create a
        // second staging row or duplicate canonical history.
        let replacement = state
            ...
            .get(&req.session_id)
            .cloned()
            .filter(|candidate| !candidate.same_channel(&sender));
        if replacement.is_some_and(|replacement| replacement.send(input).is_ok()) {
            "accepted"
        } else {
            // The message is already durable. A subsequent session start
            // promotes it in chronological order even if no replacement run
            // was available during the race.
            "queued"
        }
    };
