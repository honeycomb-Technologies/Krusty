...
async fn promote_report(
    ...
    let report_store = ReportStore::new(db.clone());
...
mod tests {
    ...
    fn seed_report(
        ...
        let session_id = session_manager
            .create_session_for_user_with_config(
                ...
                Some(project_dir.to_string_lossy().as_ref()),
                krusty_core::storage::WorkspaceMode::Selected,
                Some(user_id),
                None,
    ...
    async fn promote_report_creates_or_updates_project_memory() {
        ...
        assert_eq!(created_response.memory.memory_type, MemoryType::Project.as_str());
        ...
        let memories = store.list(Some(alice_project.to_string_lossy().as_ref()), Some("alice"));
