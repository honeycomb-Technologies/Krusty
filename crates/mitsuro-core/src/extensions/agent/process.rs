...
fn apply_baseline_environment(
    ...
    for (canonical, deprecated, value) in [
        (
            crate::identity::EXTENSION_ID_ENV,
            crate::identity::legacy::EXTENSION_ID_ENV,
            std::ffi::OsStr::new(extension_id),
        ),
        (
            crate::identity::EXTENSION_DIR_ENV,
            crate::identity::legacy::EXTENSION_DIR_ENV,
            extension_dir.as_os_str(),
        ),
        (
            crate::identity::EXTENSION_STATE_DIR_ENV,
            crate::identity::legacy::EXTENSION_STATE_DIR_ENV,
            state_dir.as_os_str(),
        ),
        (
            crate::identity::WORKING_DIR_ENV,
            crate::identity::legacy::WORKING_DIR_ENV,
...
mod tests {
    ...
    #[cfg(unix)]
    #[tokio::test]
    async fn deprecated_manifest_worker_receives_identical_baseline_environment_aliases() {
        ...
        let status = command
            .arg("-c")
            .arg(
                r#"test "$MITSURO_EXTENSION_ID" = "$KRUSTY_EXTENSION_ID" &&
test "$MITSURO_EXTENSION_DIR" = "$KRUSTY_EXTENSION_DIR" &&
test "$MITSURO_EXTENSION_STATE_DIR" = "$KRUSTY_EXTENSION_STATE_DIR" &&
test "$MITSURO_WORKING_DIR" = "$KRUSTY_WORKING_DIR""#,
            )
            .status()
            .await
            .expect("run environment contract check");
