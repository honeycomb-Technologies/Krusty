...
// This is a Codex catalog protocol compatibility version, not Mitsuro's package
// version. Keep it aligned with a stable official Codex release; operators can
// override it if the catalog raises its minimum before Mitsuro's next release.
const DEFAULT_CODEX_CLIENT_VERSION: &str = "0.144.4";
const CODEX_CLIENT_VERSION_ENV: &str = "MITSURO_CODEX_CLIENT_VERSION";
...
fn codex_client_version() -> String {
    codex_client_version_from(crate::identity::env_var(CODEX_CLIENT_VERSION_ENV).ok())
}
