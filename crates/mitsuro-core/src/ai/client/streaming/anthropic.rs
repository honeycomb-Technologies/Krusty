...
impl AiClient {
    ...
    pub(super) async fn call_streaming_anthropic(
        ...
        //   2. Base system prompt (MITSURO_SYSTEM_PROMPT) — globally stable, cached
        //   3. Frozen Hive coordinator/persona/user identity — stable per run, cached
        ...
        if let Some(cache_control) = cache_control.as_ref() {
            ...
            // the product role (operating inside Mitsuro) rather than asserting a
            ...
            if !prompt_sections.identity_context.is_empty() {
                ...
                debug!(
                    "Hive identity block added ({} chars, cached)",
