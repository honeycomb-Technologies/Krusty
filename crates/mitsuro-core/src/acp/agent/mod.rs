...
const ACP_MODEL_KEY_PREFIX: &str = "mitsuro:model-key:";
...
fn decode_acp_model_id(model_id: &str) -> Option<ModelKey> {
    let encoded = model_id
        .strip_prefix(ACP_MODEL_KEY_PREFIX)
        .or_else(|| model_id.strip_prefix(crate::identity::legacy::ACP_MODEL_KEY_PREFIX))?;
...
/// Mitsuro's ACP agent implementation.
pub struct MitsuroAgent {
    /// Session manager.
...
impl MitsuroAgent {
    /// Create a new Mitsuro ACP agent.
    pub fn new() -> Self {
    ...
    fn agent_info(&self) -> Implementation {
        Implementation::new("mitsuro", env!("CARGO_PKG_VERSION"))
    }
