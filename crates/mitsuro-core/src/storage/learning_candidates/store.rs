...
impl<'a> LearningCandidateStore<'a> {
    ...
    pub fn begin_review(
        ...
        let changed = self.db.conn().execute(
            "INSERT INTO hive_learning_runs (
             ...
             ON CONFLICT(session_id, through_message_id) DO UPDATE SET
                ...
                completed_at = NULL
             WHERE hive_learning_runs.status = 'failed'
                OR (hive_learning_runs.status = 'running'
                    AND julianday(hive_learning_runs.created_at)
