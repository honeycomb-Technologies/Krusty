...
use super::model::{
    HiveCrewProfileDocumentKind, HiveCrewProfileSnapshot, HiveProfileDocument,
...
pub const MAX_HIVE_PROFILE_DOCUMENT_BYTES: usize = 64 * 1024;

pub struct HiveProfileStore {
    db: Database,
}

#[derive(Debug, Error)]
pub enum HiveProfileStoreError {
    #[error(transparent)]
    InvalidOwner(#[from] HiveProfileOwnerError),
...
impl HiveProfileStore {
    ...
    pub fn load(
    ...
    ) -> Result<Option<HiveProfileSnapshot>, HiveProfileStoreError> {
        ...
        {
            return Err(HiveProfileStoreError::OwnerCollision { profile_id });
        }
        Ok(snapshot)
    }
    ...
    pub fn get_or_create(
        &self,
        owner: &HiveProfileOwner,
    ) -> Result<HiveProfileSnapshot, HiveProfileStoreError> {
        let tx = self.db.conn().unchecked_transaction()?;
        ensure_profile_tx(&tx, owner)?;
        tx.commit()?;
        self.load(owner)?
            .ok_or_else(|| HiveProfileStoreError::ProfileNotFound(owner.profile_id()))
    }
    ...
    pub fn update_document(
        ...
        tx.execute(
            "INSERT INTO hive_profile_documents (profile_id, kind, content, updated_at)
    ...
    pub fn update_crew_document(
        ...
        owner: &HiveProfileOwner,
        slug: &str,
        kind: HiveCrewProfileDocumentKind,
        content: &str,
        expected_revision: i64,
    ) -> Result<HiveProfileSnapshot, HiveProfileStoreError> {
        ...
        tx.execute(
            "INSERT OR IGNORE INTO hive_crew_profiles
    ...
    pub fn merge_missing(
        ...
        for (kind, content) in &seed.documents {
            ...
            let inserted = tx.execute(
                "INSERT OR IGNORE INTO hive_profile_documents
        ...
        for crew in &seed.crew {
            ...
            for (kind, content) in &crew.documents {
                let content = normalized_content(content)?;
                let inserted = tx.execute(
                    "INSERT OR IGNORE INTO hive_crew_documents
...
fn normalized_content(content: &str) -> Result<&str, HiveProfileStoreError> {
    let content = content.trim();
    if content.is_empty() {
        return Err(HiveProfileStoreError::EmptyContent);
    }
    if content.len() > MAX_HIVE_PROFILE_DOCUMENT_BYTES {
        return Err(HiveProfileStoreError::ContentTooLarge);
    }
    Ok(content)
}

fn validate_crew_slug(slug: &str) -> Result<(), HiveProfileStoreError> {
    if super::super::hive_home::is_valid_crew_slug(slug) {
        Ok(())
    } else {
        Err(HiveProfileStoreError::InvalidCrewSlug(slug.to_string()))
    }
}

fn require_revision(expected: i64, actual: i64) -> Result<(), HiveProfileStoreError> {
    if expected == actual {
        Ok(())
    } else {
        Err(HiveProfileStoreError::RevisionConflict { expected, actual })
    }
}
...
fn ensure_profile_tx(
    tx: &Transaction<'_>,
    owner: &HiveProfileOwner,
) -> Result<(), HiveProfileStoreError> {
    let profile_id = owner.profile_id();
    tx.execute(
        "INSERT OR IGNORE INTO hive_profiles
    ...
    let stored_user_id = tx
        .query_row(
            "SELECT user_id FROM hive_profiles WHERE id = ?1",
            [&profile_id],
...
fn profile_revision_tx(
    tx: &Transaction<'_>,
    profile_id: &str,
) -> Result<i64, HiveProfileStoreError> {
    tx.query_row(
        "SELECT revision FROM hive_profiles WHERE id = ?1",
        [profile_id],
        |row| row.get(0),
    )
    .optional()?
    .ok_or_else(|| HiveProfileStoreError::ProfileNotFound(profile_id.to_string()))
}
...
fn advance_profile_revision_tx(
    ...
    if changed == 1 {
        ...
        Err(HiveProfileStoreError::RevisionConflict {
...
fn load_snapshot_from_connection(
    ...
    let profile = conn
        .query_row(
            "SELECT user_id, revision FROM hive_profiles WHERE id = ?1",
            [profile_id],
    ...
    let mut snapshot = HiveProfileSnapshot {
    ...
    for row in document_rows {
        ...
        let kind = HiveProfileDocumentKind::parse(&kind)
            .ok_or_else(|| HiveProfileStoreError::InvalidStoredDocumentKind(kind.clone()))?;
        let document = Some(HiveProfileDocument {
            kind,
            content,
            updated_at,
        });
        match kind {
            HiveProfileDocumentKind::Soul => snapshot.soul = document,
            HiveProfileDocumentKind::Identity => snapshot.identity = document,
            HiveProfileDocumentKind::User => snapshot.user = document,
            HiveProfileDocumentKind::Heartbeat => snapshot.heartbeat = document,
            HiveProfileDocumentKind::Channels => snapshot.channels = document,
        }
    }

    let mut crew_stmt = conn.prepare(
        "SELECT slug, revision
         FROM hive_crew_profiles
         WHERE profile_id = ?1
         ORDER BY slug",
    )?;
    ...
    for row in crew_rows {
        ...
        let mut member = HiveCrewProfileSnapshot {
            slug: slug.clone(),
            revision,
            ..Default::default()
        };
        ...
        for document_row in crew_document_rows {
            ...
            let kind = HiveCrewProfileDocumentKind::parse(&kind).ok_or_else(|| {
                HiveProfileStoreError::InvalidStoredCrewDocumentKind(kind.clone())
            })?;
            let document = Some(HiveProfileDocument {
                kind,
                content,
            ...
            match kind {
                HiveCrewProfileDocumentKind::Identity => member.identity = document,
                HiveCrewProfileDocumentKind::Soul => member.soul = document,
            }
        }
