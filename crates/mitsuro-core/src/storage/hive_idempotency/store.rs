...
pub struct HiveIdempotencyStore {
    db: Database,
}

impl HiveIdempotencyStore {
    ...
    pub fn get(
        ...
        let sql = format!(
            "SELECT {COLUMNS} FROM hive_idempotency_keys
             WHERE scope_key = ?1 AND operation = ?2 AND idempotency_key = ?3"
        );
        self.db
            .conn()
            ...
            .optional()
            .context("reading Hive idempotency record")
    }
    ...
    pub fn delete_expired(&self, now: DateTime<Utc>) -> Result<usize> {
        let changed = self.db.conn().execute(
            "DELETE FROM hive_idempotency_keys WHERE expires_at <= ?1",
            [canonical_timestamp(now)],
        )?;
        Ok(changed)
    }
