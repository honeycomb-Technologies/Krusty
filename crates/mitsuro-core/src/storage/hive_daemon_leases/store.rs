...
pub struct HiveDaemonLeaseStore {
    db: Database,
}

impl HiveDaemonLeaseStore {
    ...
    pub fn get(&self, lease_name: &str) -> Result<Option<DaemonLease>> {
        let sql = format!("SELECT {COLUMNS} FROM hive_daemon_leases WHERE lease_name = ?1");
        self.db
            .conn()
            .query_row(&sql, [lease_name], map_lease)
            .optional()
            .context("reading Hive daemon lease")
    }
}
...
fn get_in_transaction(tx: &Transaction<'_>, lease_name: &str) -> Result<Option<DaemonLease>> {
    ...
    tx.query_row(&sql, [lease_name], map_lease)
        .optional()
        .context("reading Hive daemon lease in transaction")
}
