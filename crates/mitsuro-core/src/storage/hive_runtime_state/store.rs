...
use super::model::{HiveRunPriority, HiveRuntimeState, HiveRuntimeStateStatus};
...
pub struct HiveRuntimeStateStore {
    db: Database,
}

impl HiveRuntimeStateStore {
    pub fn new(db: Database) -> Self {
        Self { db }
    }

    pub fn get_state(&self, session_id: &str) -> Result<Option<HiveRuntimeState>> {
        let sql = format!("{SELECT_RUNTIME_STATE_COLUMNS} WHERE session_id = ?1");
        let mut stmt = self.db.conn().prepare(&sql)?;

        stmt.query_row(params![session_id], map_state_row)
            .optional()
            .context("fetching hive runtime state")
    }

    pub fn list_recoverable_states(&self) -> Result<Vec<HiveRuntimeState>> {
        let sql = format!(
    ...
    pub fn list_states_for_sessions(
        &self,
        session_ids: &[String],
    ) -> Result<HashMap<String, HiveRuntimeState>> {
    ...
    pub fn upsert_state(&self, state: &HiveRuntimeState) -> Result<()> {
    ...
    pub fn set_priority(&self, session_id: &str, priority: HiveRunPriority) -> Result<()> {
        let state = self
            .get_state(session_id)?
            .unwrap_or_else(|| HiveRuntimeState::new_empty(session_id));

        self.upsert_state(&HiveRuntimeState {
            priority,
            updated_at: chrono::Utc::now().to_rfc3339(),
            ..state
        })
    ...
    pub fn delete_state(&self, session_id: &str) -> Result<()> {
        self.db.conn().execute(
            "DELETE FROM hive_runtime_state WHERE session_id = ?1",
            params![session_id],
        )?;
        Ok(())
    }
...
fn map_state_row(row: &Row<'_>) -> rusqlite::Result<HiveRuntimeState> {
    let status_raw: String = row.get(1)?;
    let priority_raw: String = row.get(8)?;
    Ok(HiveRuntimeState {
        session_id: row.get(0)?,
        status: HiveRuntimeStateStatus::parse(&status_raw).unwrap_or(HiveRuntimeStateStatus::Idle),
        ...
        crew_slug: row.get(7)?,
        priority: HiveRunPriority::parse(&priority_raw).unwrap_or_default(),
