...
impl SessionManager {
    ...
    pub fn ensure_hive_main_session(&self, user_id: Option<&str>) -> Result<super::SessionInfo> {
        const MAIN_TITLE: &str = "Hive";

        let mut sessions = self.list_sessions_for_user_by_type(None, user_id, SessionType::Hive)?;
