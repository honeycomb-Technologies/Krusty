use super::{
    ...
    write_hive_crew_document, write_hive_home_document, HiveChannelKind, HiveCrewDocumentKind,
    HiveCrewRuntimeStatus, HiveHomeDocumentKind, HiveHomeProfile,
};
...
#[test]
fn load_hive_home_prefers_branded_top_level_files() {
    let temp = TempDir::new().unwrap();
    fs::write(temp.path().join(paths::HIVE_SOUL_FILE), "Soul.").unwrap();
    fs::write(temp.path().join(paths::HIVE_IDENTITY_FILE), "Identity.").unwrap();
    fs::write(temp.path().join(paths::HIVE_USER_FILE), "User.").unwrap();

    let profile = HiveHomeProfile::load_from(temp.path());

    assert_eq!(profile.soul.unwrap().file_name, paths::HIVE_SOUL_FILE);
    assert_eq!(
        profile.identity.unwrap().file_name,
        paths::HIVE_IDENTITY_FILE
    );
    assert_eq!(profile.user.unwrap().file_name, paths::HIVE_USER_FILE);
}

#[test]
fn load_hive_home_falls_back_to_legacy_generic_files() {
    let temp = TempDir::new().unwrap();
...
#[test]
fn load_hive_home_discovers_sorted_crew_profiles() {
    let temp = TempDir::new().unwrap();
...
#[test]
fn bootstrap_hive_home_creates_branded_files_and_default_crew() {
    let temp = TempDir::new().unwrap();

    let result = bootstrap_hive_home(temp.path()).unwrap();
    ...
    assert!(result
        .created_files
        .iter()
        .any(|path| path == paths::HIVE_USER_FILE));
    assert!(result
        .created_files
        .iter()
...
fn active_legacy_context_layers_include_user_but_exclude_memory() {
    ...
    fs::write(temp.path().join(paths::HIVE_IDENTITY_FILE), "Identity.").unwrap();
    fs::write(temp.path().join(paths::HIVE_USER_FILE), "User.").unwrap();
    fs::write(temp.path().join(paths::HIVE_MEMORY_FILE), "Legacy memory.").unwrap();
...
fn write_document_helpers_use_preferred_file_names() {
    ...
    let home_doc =
        write_hive_home_document(temp.path(), HiveHomeDocumentKind::Identity, "Hive").unwrap();
    let crew_doc = write_hive_crew_document(
        temp.path(),
        "researcher",
    ...
    assert_eq!(home_doc.file_name, paths::HIVE_IDENTITY_FILE);
    assert_eq!(crew_doc.file_name, "SOUL.md");
    assert!(temp.path().join(paths::HIVE_IDENTITY_FILE).is_file());
    assert!(temp
        .path()
        .join("crew")
        .join("researcher")
...
fn summarize_crew_runtime_merges_profile_tasks_and_delegated_runs() {
    let temp = TempDir::new().unwrap();
    bootstrap_hive_home(temp.path()).unwrap();
    ...
    let builder = summary
        ...
        .unwrap();
    assert_eq!(builder.status, HiveCrewRuntimeStatus::Running);
    ...
    let reviewer = summary
        ...
        .unwrap();
    assert_eq!(reviewer.status, HiveCrewRuntimeStatus::Degraded);
...
fn summarize_channel_bindings_combines_system_and_home_entries() {
    ...
    bootstrap_hive_home(temp.path()).unwrap();
    fs::write(
            temp.path().join(paths::HIVE_CHANNELS_FILE),
    ...
    assert_eq!(push.kind, HiveChannelKind::MobilePush);
    assert!(push.enabled);
