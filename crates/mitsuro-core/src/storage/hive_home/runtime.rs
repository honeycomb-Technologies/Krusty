...
use crate::storage::{
    AutonomousTaskStore, DelegatedRunStore, HiveRuntimeState, HiveRuntimeStateStatus, SessionInfo,
    TaskStatus,
};

use super::{HiveCrewRuntimeStatus, HiveCrewRuntimeSummary, HiveHomeProfile};
use crate::agent::DelegatedRunStage;

pub fn summarize_crew_runtime(
    profile: &HiveHomeProfile,
    sessions: &[SessionInfo],
    runtime_states: &HashMap<String, HiveRuntimeState>,
    task_store: &AutonomousTaskStore,
    delegated_store: &DelegatedRunStore,
) -> Result<Vec<HiveCrewRuntimeSummary>> {
    let known_slugs = profile
        .crew
        .iter()
        .map(|member| normalize_agent_key(&member.slug))
        .collect::<HashSet<_>>();
    let mut summaries = BTreeMap::<String, HiveCrewRuntimeSummary>::new();

    for member in &profile.crew {
        summaries.insert(
            normalize_agent_key(&member.slug),
            HiveCrewRuntimeSummary {
...
fn new_runtime_summary(slug: &str, known_to_home: bool) -> HiveCrewRuntimeSummary {
    HiveCrewRuntimeSummary {
...
fn resolve_runtime_status(summary: &HiveCrewRuntimeSummary) -> HiveCrewRuntimeStatus {
    if summary.active_run_count > 0 || summary.active_task_count > 0 {
        HiveCrewRuntimeStatus::Running
    } else if summary.failed_run_count > 0 || summary.failed_task_count > 0 {
        HiveCrewRuntimeStatus::Degraded
    } else if summary.queued_task_count > 0 {
        HiveCrewRuntimeStatus::Waiting
    } else {
        HiveCrewRuntimeStatus::Idle
    }
}
