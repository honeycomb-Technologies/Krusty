...
use crate::hive::{normalize_timestamp, parse_timezone};
use crate::storage::Database;

use super::{HiveController, HiveControllerStatus};

const CONTROLLER_COLUMNS: &str = "id, scope_key, user_id, session_id, status, timezone, max_concurrent_runs, created_at, updated_at";

pub struct HiveControllerStore {
    db: Database,
}

impl HiveControllerStore {
    pub fn new(db: Database) -> Self {
        Self { db }
    }

    pub fn insert(&self, controller: &HiveController) -> Result<()> {
        ...
        self.db
            ...
            .context("inserting Hive controller")?;
        Ok(())
    }

    pub fn get(&self, id: &str) -> Result<Option<HiveController>> {
        self.query_one("id", id)
    }

    pub fn get_by_scope(&self, scope_key: &str) -> Result<Option<HiveController>> {
        self.query_one("scope_key", scope_key)
    }

    pub fn get_by_session(&self, session_id: &str) -> Result<Option<HiveController>> {
    ...
    fn query_one(&self, column: &str, value: &str) -> Result<Option<HiveController>> {
        debug_assert!(matches!(column, "id" | "scope_key" | "session_id"));
        let sql = format!("SELECT {CONTROLLER_COLUMNS} FROM hive_controllers WHERE {column} = ?1");
        self.db
            .conn()
            .query_row(&sql, [value], map_controller)
            .optional()
            .context("reading Hive controller")
    }
...
fn map_controller(row: &Row<'_>) -> rusqlite::Result<HiveController> {
    ...
    Ok(HiveController {
        ...
        status: HiveControllerStatus::parse(&status).ok_or_else(|| {
            rusqlite::Error::FromSqlConversionFailure(
                4,
                rusqlite::types::Type::Text,
                Box::new(IoError::new(
                    ErrorKind::InvalidData,
                    format!("invalid Hive controller status: {status}"),
                )),
            )
        })?,
        timezone: row.get(5)?,
