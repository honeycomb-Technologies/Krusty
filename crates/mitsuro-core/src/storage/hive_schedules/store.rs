        ...
        let rows = statement
            .query_map(params![user_id, limit as i64], |row| {
                Ok(OwnedHiveSchedule {
                    ...
                    controller_session_id: row.get(33)?,
                })
            })?
            .collect::<rusqlite::Result<Vec<_>>>()
    ...
    anyhow::ensure!(
        !(schedule
            ...
            && schedule
                ...
                .as_deref()
                .is_some_and(|id| !id.trim().is_empty())),
        "schedule cannot target both a Worker and a Group"
    );
    Ok(())
}
