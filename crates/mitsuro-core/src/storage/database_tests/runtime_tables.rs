...
#[test]
fn test_hive_runtime_state_table_exists() {
    let (db, _temp) = create_test_db();

    let conn = db.conn();
    let mut stmt = conn
        .prepare("SELECT name FROM sqlite_master WHERE type='table' AND name='hive_runtime_state'")
        .expect("Failed to prepare query");
    ...
    assert!(tables.contains(&"hive_runtime_state".to_string()));

    let mut stmt = conn
        .prepare("PRAGMA table_info(hive_runtime_state)")
        .expect("Failed to prepare PRAGMA");
...
#[test]
fn test_hive_attention_state_table_exists() {
    let (db, _temp) = create_test_db();

    let conn = db.conn();
    let mut stmt = conn
        .prepare(
            "SELECT name FROM sqlite_master WHERE type='table' AND name='hive_attention_state'",
        )
        .expect("Failed to prepare query");
    ...
    assert!(tables.contains(&"hive_attention_state".to_string()));

    let mut stmt = conn
        .prepare("PRAGMA table_info(hive_attention_state)")
        .expect("Failed to prepare PRAGMA");
