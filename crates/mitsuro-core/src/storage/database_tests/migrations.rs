...
fn provider_aware_model_identity_columns_are_additive() {
    ...
    let schedule_columns: Vec<String> = db
        ...
        .query_map([], |row| row.get(1))
        .expect("query Hive schedule columns")
        .collect::<rusqlite::Result<_>>()
        .expect("collect Hive schedule columns");
...
fn migration_45_upgrades_schema_44_without_rewriting_legacy_model() {
    ...
    let db = crate::storage::database::Database::open_migration_fixture(&path, 45)
        .expect("apply only migration 45 to the minimal schema 44 fixture");
    assert_eq!(db.get_schema_version(), 45);
...
fn migration_46_upgrades_schema_45_without_guessing_legacy_schedule_identity() {
    ...
    let db = crate::storage::database::Database::open_migration_fixture(&path, 46)
        .expect("apply only migration 46 to the minimal schema 45 fixture");
