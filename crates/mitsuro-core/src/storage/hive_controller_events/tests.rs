...
use super::{HiveControllerEventStore, HiveControllerEventType, NewHiveControllerEvent};

fn store() -> (HiveControllerEventStore, TempDir) {
    ...
    db.conn()
        ...
        .unwrap();
    (HiveControllerEventStore::new(db), temp)
}

fn event(dedupe_key: Option<&str>, run_id: &str) -> NewHiveControllerEvent {
    NewHiveControllerEvent {
        controller_id: "controller-1".into(),
        event_type: HiveControllerEventType::RunQueued,
        run_id: None,
        schedule_id: None,
...
fn dedupe_key_returns_the_original_event_without_advancing_sequence() {
    ...
    assert_eq!(
        store
            .conn()
            .query_row("SELECT COUNT(*) FROM hive_controller_events", [], |row| {
                row.get::<_, i64>(0)
            })
            .unwrap(),
        2
