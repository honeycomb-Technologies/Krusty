...
use crate::hive::normalize_timestamp;
use crate::storage::Database;

use super::{HiveControllerEvent, NewHiveControllerEvent};

const COLUMNS: &str = "id, controller_id, sequence, event_type, run_id, schedule_id, dedupe_key, payload_json, created_at";

pub struct HiveControllerEventStore {
    db: Database,
}

impl HiveControllerEventStore {
    pub fn new(db: Database) -> Self {
        Self { db }
    }

    /// Appends a controller-local ordered event. A duplicate dedupe key returns the first event.
    pub fn append(&self, new_event: &NewHiveControllerEvent) -> Result<HiveControllerEvent> {
        anyhow::ensure!(
        ...
        let sql = format!("SELECT {COLUMNS} FROM hive_controller_events WHERE id = ?1");
    ...
    pub fn list_after(
        ...
        let events = statement
            ...
            .context("listing Hive controller events")?;
        Ok(events)
    }
    ...
    pub fn list_pending_tool_approvals(
        ...
        let events = statement
            ...
            .collect::<rusqlite::Result<Vec<_>>>()
            .context("listing pending durable Hive tool approvals")?;
        Ok(events)
    }
    ...
    pub fn list_pending_user_response_runs(
        ...
        let run_ids = statement
            ...
            .collect::<rusqlite::Result<Vec<_>>>()
            .context("listing pending durable Hive user responses")?;
        Ok(run_ids)
    }
...
fn get_by_dedupe(
    ...
    controller_id: &str,
    dedupe_key: &str,
) -> Result<Option<HiveControllerEvent>> {
    let sql = format!(
        "SELECT {COLUMNS} FROM hive_controller_events
...
fn map_event(row: &Row<'_>) -> rusqlite::Result<HiveControllerEvent> {
