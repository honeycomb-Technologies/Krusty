...
async fn parses_remote_oauth_pkce_configuration() {
            ...
            "mcpServers": {
                "remote": {
                    ...
                    "oauth": {
                        ...
                        "clientName": "Mitsuro Test Client"
                    }
                }
            }
        }"#,
    ...
    assert_eq!(oauth.client_name(), "Mitsuro Test Client");
}
...
async fn untrusted_expansion_never_reads_the_host_environment() {
    let variable = format!("MITSURO_MCP_SECRET_{}", std::process::id());
...
async fn project_remote_servers_do_not_auto_connect_or_inherit_host_secrets() {
    let dir = tempfile::tempdir().unwrap();
    let variable = format!("MITSURO_MCP_PROJECT_SECRET_{}", std::process::id());
    std::env::set_var(&variable, "host-secret");
    tokio::fs::write(
        dir.path().join(".mcp.json"),
        format!(
...
async fn package_remote_servers_do_not_auto_connect_or_inherit_host_secrets() {
    ...
    let variable = format!("MITSURO_MCP_PACKAGE_SECRET_{}", std::process::id());
    std::env::set_var(&variable, "host-secret");
    tokio::fs::write(
        &package,
        format!(
