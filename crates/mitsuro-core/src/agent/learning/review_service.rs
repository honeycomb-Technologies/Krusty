...
/// Exact-owner review boundary for Hive's pending durable-learning proposals.
///
/// Acceptance validates the current deterministic policy and original Hive
...
fn validate_exact_evidence(
    ...
    if role != "user" || session_type != "hive" {
        return Err(LearningReviewServiceError::InvalidEvidence(
            "the cited evidence must be a canonical user message in a Hive session".to_string(),
        ));
    }
...
mod tests {
    ...
    fn policy_or_evidence_failure_leaves_candidate_pending_without_memory() {
        ...
        db.conn()
            .execute(
                "UPDATE hive_learning_candidates SET evidence_excerpt = 'not in the message'
                 WHERE id = ?1",
