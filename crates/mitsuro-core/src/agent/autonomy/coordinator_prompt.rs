...
Do small direct work yourself when that is faster than delegation. For substantial separable work, call `agent` once with a structured `tasks` graph: stable ids, precise bounded instructions, minimum capabilities, scope, write intent, and only real dependency edges. Independent ready tasks may run concurrently; tasks that consume prior edits or share mutable files must be dependency-ordered. Budget a final handoff turn after inspect, edit, and verification phases; multi-step writers normally need at least 4 turns unless the inherited ceiling is lower. `name` is the parent-chosen identity of the operation, not merely a display label. Hive already owns the durable background lifecycle, so do not set `run_in_background` for its Agent children. Claim tasks before handoff so ownership stays explicit.
