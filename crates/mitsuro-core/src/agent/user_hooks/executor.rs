...
impl UserHookExecutor {
    ...
    pub async fn execute(
        ...
        {
            command
                .env("MITSURO_PLUGIN_ID", plugin_id)
                .env("MITSURO_HOOK_CONFIG", config_path)
                .env(crate::identity::legacy::PACKAGE_PLUGIN_ID_ENV, plugin_id)
                .env(
                    crate::identity::legacy::PACKAGE_HOOK_CONFIG_ENV,
                    config_path,
                );
            if let Some(package_root) = hook.working_dir() {
                command
                    .env("MITSURO_PLUGIN_ROOT", package_root)
                    .env(
                        crate::identity::legacy::PACKAGE_PLUGIN_ROOT_ENV,
                        package_root,
                    )
...
mod tests {
    ...
    async fn test_package_hook_runs_from_package_root() {
        ...
        let hook = UserHook::new_package(
            ...
            ".*".to_string(),
            r#"test -f marker &&
test "$MITSURO_PLUGIN_ID" = "$KRUSTY_PLUGIN_ID" &&
test "$MITSURO_HOOK_CONFIG" = "$KRUSTY_HOOK_CONFIG" &&
test "$MITSURO_PLUGIN_ROOT" = "$KRUSTY_PLUGIN_ROOT" &&
test "$MITSURO_PLUGIN_ROOT" = "$(pwd)" &&
test "$CLAUDE_PLUGIN_ROOT" = "$(pwd)""#
                .to_string(),
            true,
            30,
            "test".to_string(),
