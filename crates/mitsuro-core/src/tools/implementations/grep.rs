...
impl Tool for GrepTool {
    ...
    fn prompt(&self) -> Option<&str> {
        Some(
            r#"Use instead of shell grep/rg for file-content search. Supports ripgrep regex syntax.

Output modes: content (default), files_with_matches, count. Use head_limit to cap results and glob/type to filter files.

Use multiline:true for multi-line patterns; -C for context; -i for case-insensitive search."#,
        )
    }
