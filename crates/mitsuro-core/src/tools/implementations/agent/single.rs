...
fn persisted_target_matches(previous: &[DelegatedRunScope], current: &[DelegatedRunScope]) -> bool {
    let ([previous], [current]) = (previous, current) else {
        return false;
    };
    let kind_matches = previous.kind == current.kind
        || (previous.kind == "project"
            && current.kind == "directory"
            && normalize_persisted_target(&previous.path).is_empty()
            && normalize_persisted_target(&current.path).is_empty());
    kind_matches
        && normalize_persisted_target(&previous.path) == normalize_persisted_target(&current.path)
}
                ...
                match store.get_run(resumed_from_run_id) {
                    ...
                    Ok(Some(_)) => {
                        return ToolResult::error_with_code(
                            ...
                            "The selected delegated run no longer matches this session or capability contract.",
                        );
                    }
                    Ok(None) => {
                        return ToolResult::error_with_code(
        ...
        if let (Some(store), Some(session_id)) = (delegated_store.as_ref(), ctx.session_id.as_ref())
        {
            if let Err(err) = store.create_run_with_child_contract(
                &DelegatedRunStartInput {
                    ...
                    resumable: true,
                    resumed_from_run_id: params.resumed_from_run_id.clone().or_else(|| {
                        resume_candidate
                            .as_ref()
...
mod child_scope_tests {
    ...
    fn file_target_persists_the_file_not_its_working_directory() {
        ...
        assert_eq!(scope.path, "src/auth/mod.rs");
        assert_eq!(scope.kind, "file");
        assert!(persisted_target_matches(
            std::slice::from_ref(&scope),
            std::slice::from_ref(&scope)
        ));

        let malformed = DelegatedRunScope {
            label: "auth/mod.rs".to_string(),
            path: "src/auth".to_string(),
            kind: "file".to_string(),
        };
        let widened = DelegatedRunScope {
            label: "src/auth".to_string(),
            path: "src/auth".to_string(),
            kind: "directory".to_string(),
        };
        assert!(!persisted_target_matches(&[malformed], &[widened]));
    }
}
