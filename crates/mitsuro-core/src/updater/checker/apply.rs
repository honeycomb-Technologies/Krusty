...
pub fn apply_pending_update() -> Result<Option<String>> {
    ...
    enforce_pending_apply_policy(&pending, &version_path)?;
...
fn enforce_pending_apply_policy(pending: &Path, version_path: &Path) -> Result<()> {
...
mod tests {
    ...
    #[test]
    fn pending_apply_fails_closed_and_discards_both_artifacts_on_every_platform() {
        let dir = tempfile::tempdir().expect("tempdir");
        let pending = dir.path().join("pending");
        let version = dir.path().join("pending.version");
        fs::write(&pending, b"untrusted replacement").expect("write pending");
        fs::write(&version, b"99.0.0").expect("write version");

        let error = enforce_pending_apply_policy(&pending, &version)
            .expect_err("single-binary apply must fail closed");
    ...
    fn stale_replacement_artifacts_are_reconciled_idempotently() {
        let dir = tempfile::tempdir().expect("tempdir");
        let current = dir.path().join("mitsuro.exe");
        let backup = dir.path().join("mitsuro.exe.old");
        let staged = dir.path().join("mitsuro.new");
