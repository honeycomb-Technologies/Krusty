...
use crate::updater::checker::checksum::{
    parse_published_sha256, verify_archive_sha256, MAX_CHECKSUM_BYTES,
};
...
pub(super) async fn download_update_release(
    ...
    let archive_name = format!("mitsuro-{}.{}", platform, ext);
    let url = format!(
...
mod tests {
    ...
    const ARCHIVE_NAME: &str = "mitsuro-x86_64-pc-windows-msvc.zip";
    ...
    fn stale_archive_cleanup_is_idempotent() {
        ...
        let archive = dir.path().join("mitsuro-download.zip");
