...
pub(super) fn extract_tar_gz(archive: &Path, dest: &Path) -> Result<()> {
    ...
    let extracted_binary = extract_dir.join("mitsuro");
    if !extracted_binary.exists() {
        ...
        return Err(anyhow!("Binary 'mitsuro' not found in archive"));
    }
...
pub(super) fn extract_zip(archive: &Path, dest: &Path) -> Result<()> {
    debug!(
        "Extracting exact mitsuro.exe entry from {}",
        archive.display()
    );
...
fn extract_exact_windows_binary(archive: &Path, staged: &Path) -> Result<()> {
    ...
    if zip.len() != 1 {
        return Err(anyhow!(
            "Release ZIP must contain exactly one mitsuro.exe entry, found {}",
            zip.len()
        ));
    }
    ...
    if entry.name() != "mitsuro.exe" || enclosed_name != Path::new("mitsuro.exe") {
        return Err(anyhow!(
            "Release ZIP entry must be exactly 'mitsuro.exe', found '{}'",
            entry.name()
        ));
    }
    if !entry.is_file() {
    ...
    .context("failed to extract mitsuro.exe")?;
...
mod tests {
    ...
    fn extracts_only_exact_windows_binary_entry() {
        ...
        write_zip(&archive, &[("mitsuro.exe", b"binary")]);
    ...
    fn rejects_zip_with_unexpected_or_traversing_entries() {
        ...
        write_zip(
            &extra_archive,
            &[("mitsuro.exe", b"binary"), ("extra.txt", b"unexpected")],
        );
        write_zip(&traversal_archive, &[("../mitsuro.exe", b"binary")]);
