...
pub enum PluginRuntime {
    /// Native dynamic library plugin using Mitsuro's C ABI.
    Native,
    ...
    /// today. Executable Wasmtime isolation belongs to Mitsuro's separate
...
impl PluginRuntime {
    ...
    /// today, so accepting one does not require process permission. Mitsuro's
...
pub struct PluginCompat {
    // These serialized keys are authenticated bytes in the immutable
    // manifest-envelope-v1 protocol. The canonical spellings remain read
    // aliases until a separately versioned envelope changes the wire format.
    #[serde(default, rename = "krusty_min", alias = "mitsuro_min")]
    pub mitsuro_min: Option<String>,
    #[serde(default, rename = "krusty_max", alias = "mitsuro_max")]
    pub mitsuro_max: Option<String>,
}
...
pub enum PluginSourceTrust {
    ...
    SignedPublisher,
    /// Installed from npm. Registry transport is trusted, package code is not signed by Mitsuro.
    NpmUnsigned,
...
pub struct PluginSource {
    pub name: String,
    /// URL or local path to a Mitsuro plugin catalog file.
    ///
