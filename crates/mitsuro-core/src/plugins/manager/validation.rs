...
pub(super) fn validate_compatibility(compat: &PluginCompat) -> Result<()> {
    let current = Version::parse(env!("CARGO_PKG_VERSION")).with_context(|| {
        format!(
            "failed to parse current mitsuro version '{}'",
    ...
    let min = compat
        ...
        .map(|value| {
            Version::parse(value)
                .with_context(|| format!("invalid compat.mitsuro_min version '{}'", value))
        })
        .transpose()?;
    let max = compat
        .mitsuro_max
        ...
        .map(|value| {
            Version::parse(value)
                .with_context(|| format!("invalid compat.mitsuro_max version '{}'", value))
        })
        .transpose()?;
    ...
    if let (Some(min), Some(max)) = (&min, &max) {
        if min > max {
            bail!(
                "invalid compat range: compat.mitsuro_min ({}) is greater than compat.mitsuro_max ({})",
                min,
                max
            );
        }
    ...
    if let Some(min) = min {
        if current < min {
            bail!(
                "plugin requires mitsuro >= {}, current version is {}",
                min,
                current
            );
        }
