...
#[tokio::test]
async fn rejects_incompatible_mitsuro_version_bounds() {
    let temp = tempdir().expect("tempdir");
    let workspace = temp.path();
    let manifest_dir = workspace.join("manifest");
    fs::create_dir_all(&manifest_dir)
...
"#
    ...
    let err = manager
        ...
        .expect_err("install should fail");
    assert!(
        err.to_string().contains("requires mitsuro >="),
        "unexpected error: {}",
        err
    );
}
...
async fn installs_local_package_declared_by_package_json() {
    ...
    fs::write(
        ...
        r#"{
  "name": "@mitsuro/demo-plugin",
  "version": "1.0.0",
  "mitsuro": { "plugins": ["./plugin.toml"] }
}"#,
    )
    .await
  ...
  "plugins": [
    {
      ...
      "package": "npm:@mitsuro/catalog-demo",
      "runtime": "wasm",
...
"#,
    ...
    fs::write(
        ...
        "mitsuro.registerPlugin({ renderText() { return ['hi']; } });",
    )
    .await
...
async fn uninstall_keeps_shared_snapshot_until_last_plugin_is_removed() {
    ...
    fs::write(
        package_dir.join("package.json"),
        r#"{"mitsuro":{"plugins":["a.toml","b.toml"]}}"#,
    )
    .await
    .expect("write package json");
...
async fn package_manifest_fanout_and_duplicate_paths_fail_early() {
    ...
    fs::write(
        ...
        serde_json::to_vec(&serde_json::json!({ "mitsuro": { "plugins": paths } })).unwrap(),
    )
    .await
    ...
    fs::write(
        ...
        br#"{"mitsuro":{"plugins":["missing.toml","./missing.toml"]}}"#,
    )
    .await
