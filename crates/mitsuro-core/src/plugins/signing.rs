...
// `manifest-envelope-v1` is an immutable signature protocol. The historical
// bytes are not branding and cannot change without invalidating every existing
// signed manifest. A canonical domain requires a version-2 scheme.
const LEGACY_RELEASE_ENVELOPE_V1_DOMAIN: &[u8] = b"krusty-plugin-release-envelope-v1\0";
...
pub fn plugin_release_signing_payload(manifest: &PluginManifestV1) -> Result<Vec<u8>> {
    ...
    let mut payload = Vec::with_capacity(LEGACY_RELEASE_ENVELOPE_V1_DOMAIN.len() + json.len());
    payload.extend_from_slice(LEGACY_RELEASE_ENVELOPE_V1_DOMAIN);
...
mod tests {
    ...
    #[test]
    fn manifest_envelope_v1_keeps_exact_deprecated_domain_and_json_keys() {
        let manifest: PluginManifestV1 =
            serde_json::from_value(manifest_value()).expect("parse manifest");
        let payload = plugin_release_signing_payload(&manifest).expect("signing payload");
        let expected_json = format!(
            r#"{{"manifest_version":1,"id":"signed.plugin","name":"Signed Plugin","version":"1.0.0","publisher":"signed.publisher","description":null,"runtime":"wasm","entry_component":"plugin.wasm","render_capabilities":[],"requested_permissions":{{"fs_read":false,"fs_write":false,"network":false,"process":false}},"release":{{"url":"https://plugins.example/plugin.wasm","sha256":"{}","signature":"","signing_key_id":"release-key","signature_scheme":"manifest-envelope-v1"}},"compat":{{"krusty_min":"0.7.0","krusty_max":null}}}}"#,
            "00".repeat(32)
        );
        let mut expected = LEGACY_RELEASE_ENVELOPE_V1_DOMAIN.to_vec();
    ...
    #[test]
    fn manifest_envelope_v1_verifies_a_fixed_deprecated_signature_vector() {
        let manifest: PluginManifestV1 =
            serde_json::from_value(manifest_value()).expect("parse manifest");
        let payload = plugin_release_signing_payload(&manifest).expect("signing payload");
        ...
        assert_eq!(
            signature_b64,
            "2+6E2apkj6swixwyh+psrErXHzxReBm99kfmW1gMIJVkvjQj5vjvMWcU+m6HJEk+7znrY0WxW3NEMnx4+Js7Cg=="
        );
        verify_artifact_signature(&payload, &signature_b64, &public_key_b64)
            .expect("fixed signature must verify");
    }
    ...
    fn release_artifact_kind_is_authenticated_without_changing_legacy_default_payloads() {
        ...
        let default_json: serde_json::Value =
            serde_json::from_slice(&default_payload[LEGACY_RELEASE_ENVELOPE_V1_DOMAIN.len()..])
        ...
        let bundle_json: serde_json::Value =
            serde_json::from_slice(&bundle_payload[LEGACY_RELEASE_ENVELOPE_V1_DOMAIN.len()..])
