...
//! for exposing the Mitsuro server with automatic HTTPS on the tailnet.
//!
...
/// Dedicated HTTPS port for Mitsuro on Tailscale.
...
/// Outcome of attempting to configure Tailscale serve for Mitsuro.
#[derive(Debug, Clone)]
...
/// Get the HTTPS URL for Mitsuro on the tailnet (port 8443).
...
///
/// Uses port 8443 so Mitsuro doesn't clash with other services on port 443.
...
/// Stop `tailscale serve` for the mitsuro HTTPS port.
