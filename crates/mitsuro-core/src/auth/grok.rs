...
/// Convert a grok-auth token to Mitsuro's generic OAuth token shape for UI status.
...
/// Ensure Grok auth is fresh and mirror the status into Mitsuro's OAuth store.
