...
use mitsuro_core::agent::{ActionClass, ProgressLedger};
use mitsuro_core::ai::types::{AiToolCall, Content};
...
fn production_progress_guard_converges_cosmetic_grok_bash_variants() {
    ...
    for (index, command) in commands.iter().enumerate() {
        ...
        let fingerprint = mitsuro_core::agent::progress::action_fingerprint(&call);
