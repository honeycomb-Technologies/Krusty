...
// Protocol v1 domains are immutable signed bytes. They intentionally retain
// their original spelling so new clients can authenticate to old daemons and
// vice versa. A canonical Hive domain requires a negotiated protocol v2.
const LEGACY_V1_HELLO_DOMAIN: &str = "mako-ipc-hello-v1";
const LEGACY_V1_ACK_DOMAIN: &str = "mako-ipc-ack-v1";
...
fn create_private_key_candidate(parent: &Path) -> Result<(IpcKey, TemporaryKeyPath), AuthError> {
    ...
    for _ in 0..MAX_TEMP_ATTEMPTS {
        ...
        let path = parent.join(format!(".hive-ipc-key-{}.tmp", encode_hex(&suffix)));
...
/// Create or tighten the private directory used for Hive socket/key material.
...
mod tests {
    ...
    #[test]
    fn protocol_v1_signing_domains_remain_byte_exact() {
        assert_eq!(
            hello_signing_input(ProtocolVersion { major: 1, minor: 2 }, "client", "nonce", 7),
            b"mako-ipc-hello-v1\n1.2\nclient\nnonce\n7"
        );
        assert_eq!(
            ack_signing_input(
    ...
    fn creates_private_key_and_directory() {
        ...
        let key_path = directory.join("hive.ipc.key");
