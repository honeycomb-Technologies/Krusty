...
#[derive(Debug, Clone)]
pub struct HiveIpcClientConfig {
...
#[derive(Clone)]
pub struct HiveIpcClient {
    config: HiveIpcClientConfig,
    key: Arc<IpcKey>,
}

impl std::fmt::Debug for HiveIpcClient {
    fn fmt(&self, formatter: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        formatter
            .debug_struct("HiveIpcClient")
...
impl HiveIpcClient {
    pub fn new(config: HiveIpcClientConfig, key: IpcKey) -> Self {
        Self {
            config,
            key: Arc::new(key),
        }
