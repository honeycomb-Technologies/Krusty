...
impl App {
    pub(super) fn handle_goal_command(&mut self, subcommand: Option<&str>) {
        ...
        let manager = match mitsuro_core::workflow::WorkflowManager::new(db_path) {
            Ok(manager) => manager,
            Err(error) => {
        ...
        let snapshot = match manager.get_snapshot(&session_id) {
            ...
            Ok(None) => {
                self.goal_message(
                    "No durable Goal. Enter Plan mode and ask Mitsuro to draft one.".to_string(),
                );
                return;
            }
            Err(error) => {
        ...
        match result {
            Ok(mutation) => {
                if mutation.snapshot.goal.status == mitsuro_core::workflow::GoalStatus::Active {
