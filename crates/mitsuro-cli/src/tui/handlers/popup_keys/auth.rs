        ...
        match method {
            AuthMethod::OAuthBrowser => {
                ...
                if provider == ProviderId::Anthropic {
                    ...
                    match flow.get_auth_url() {
                        Ok((url, verifier, _state)) => {
                            // Open browser
                            if let Err(e) = mitsuro_core::auth::open_browser(&url) {
                                self.ui
                                    .popups
                                    .auth
                ...
                if provider == ProviderId::Grok {
                    tokio::spawn(async move {
                        match mitsuro_core::auth::force_grok_browser_login().await {
                            Ok(token) => {
                                let _ = status_tx.send(OAuthStatusUpdate {
                                    provider,
                                    success: true,
                                    message: "Authentication successful".to_string(),
                                    device_code: None,
                                    token: Some(mitsuro_core::auth::grok_auth_token_to_oauth_data(
                                        &token,
                                    )),
                                });
                            }
