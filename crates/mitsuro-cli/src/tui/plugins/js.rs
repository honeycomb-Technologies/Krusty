...
const JS_RENDER_HELP: &str =
    "JS/TS plugins must call mitsuro.registerPlugin({ renderText() { return ['line']; } }).";
...
impl JsPluginHost {
    ...
    fn load_inner(&self) -> anyhow::Result<edon::Nodejs> {
        let libnode_path = resolve_libnode_path().ok_or_else(|| {
            anyhow::anyhow!(
                "set MITSURO_LIBNODE or EDON_LIBNODE_PATH to libnode.so/libnode.dylib/libnode.dll"
            )
        })?;
        ...
        let prelude = format!(
            r#"
globalThis.__mitsuroPluginRegistry = globalThis.__mitsuroPluginRegistry || Object.create(null);
globalThis.__mitsuroPluginLogs = globalThis.__mitsuroPluginLogs || Object.create(null);
// Deprecated registry/log aliases keep already-installed plugins operational.
globalThis.__krustyPluginRegistry = globalThis.__mitsuroPluginRegistry;
globalThis.__krustyPluginLogs = globalThis.__mitsuroPluginLogs;
globalThis.__mitsuroPluginLogs[{plugin_id}] = [];
globalThis.console = {{
  log: (...args) => globalThis.__mitsuroPluginLogs[{plugin_id}].push(args.map(String).join(' ')),
  warn: (...args) => globalThis.__mitsuroPluginLogs[{plugin_id}].push(args.map(String).join(' ')),
  error: (...args) => globalThis.__mitsuroPluginLogs[{plugin_id}].push(args.map(String).join(' ')),
}};
globalThis.mitsuro = globalThis.mitsuro || {{}};
globalThis.mitsuro.registerPlugin = function(plugin) {{
  globalThis.__mitsuroPluginRegistry[{plugin_id}] = plugin;
}};
globalThis.krusty = globalThis.mitsuro;
"#
        );
        let postlude = format!(
            r#"
if (globalThis.mitsuroPlugin) {{
  globalThis.__mitsuroPluginRegistry[{plugin_id}] = globalThis.mitsuroPlugin;
}} else if (globalThis.krustyPlugin) {{
  globalThis.__mitsuroPluginRegistry[{plugin_id}] = globalThis.krustyPlugin;
}}
"#
        );
    ...
    fn refresh_render_lines(&mut self) {
        ...
        let script = format!(
            r#"
const fs = require('fs');
const plugin = globalThis.__mitsuroPluginRegistry && globalThis.__mitsuroPluginRegistry[{plugin_id}];
let lines = [];
...
fn resolve_libnode_path() -> Option<PathBuf> {
    mitsuro_core::identity::env_var_os("MITSURO_LIBNODE")
...
mod tests {
    ...
    fn runs_ts_plugin_when_libnode_available() {
        if mitsuro_core::identity::env_var_os("MITSURO_LIBNODE").is_none()
