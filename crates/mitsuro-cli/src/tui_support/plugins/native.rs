...
//! executing arbitrary local code. Mitsuro keeps the ABI small and C-compatible so
...
use super::{
    kitty_graphics::PluginFrame, legacy_identity, InstalledPluginDescriptor, Plugin, PluginContext,
    PluginEventResult, PluginRenderMode,
};

pub const MITSURO_NATIVE_PLUGIN_ABI_VERSION: u32 = 1;
pub const MITSURO_NATIVE_EVENT_KEY: u32 = 1;
pub const MITSURO_NATIVE_EVENT_ENTER: u32 = 0x1_0000;
pub const MITSURO_NATIVE_EVENT_ESC: u32 = 0x1_0001;
pub const MITSURO_NATIVE_EVENT_BACKSPACE: u32 = 0x1_0002;
pub const MITSURO_NATIVE_EVENT_UP: u32 = 0x1_0003;
pub const MITSURO_NATIVE_EVENT_DOWN: u32 = 0x1_0004;
pub const MITSURO_NATIVE_EVENT_LEFT: u32 = 0x1_0005;
pub const MITSURO_NATIVE_EVENT_RIGHT: u32 = 0x1_0006;

pub const MITSURO_NATIVE_EVENT_RESULT_CONSUMED: u32 = 1;
...
type HandleEventFn = unsafe extern "C" fn(instance: *mut c_void, event: MitsuroNativeEvent) -> u32;
...
impl NativePluginInstance {
    fn load(descriptor: &InstalledPluginDescriptor) -> anyhow::Result<Self> {
        ...
        let api = unsafe {
            let entry: libloading::Symbol<unsafe extern "C" fn() -> *const MitsuroNativePluginV1> =
                match library.get(b"mitsuro_plugin_entry") {
                    Ok(entry) => entry,
                    Err(canonical_error) => library
                        .get(legacy_identity::NATIVE_PLUGIN_ENTRY_SYMBOL)
                        .map_err(|legacy_error| {
                            anyhow::anyhow!(
                                "native plugin exports neither canonical nor deprecated entry point: canonical={canonical_error}; deprecated={legacy_error}"
                            )
                        })?,
                };
            let api_ptr = entry();
            if api_ptr.is_null() {
                anyhow::bail!("native plugin entry point returned null");
            }
            *api_ptr
        };

        if api.abi_version != MITSURO_NATIVE_PLUGIN_ABI_VERSION {
            anyhow::bail!(
                "unsupported native plugin ABI {}; expected {}",
                api.abi_version,
                MITSURO_NATIVE_PLUGIN_ABI_VERSION
            );
        }
    ...
    fn handle_event(&mut self, event: &Event) -> PluginEventResult {
        ...
        if result == MITSURO_NATIVE_EVENT_RESULT_CONSUMED {
...
fn shadow_copy_native_library(
    ...
    let shadow_dir = install_path.join(".mitsuro-shadow");
...
fn map_event(event: &Event) -> Option<MitsuroNativeEvent> {
    ...
    let key_code = match code {
        ...
        KeyCode::Enter => MITSURO_NATIVE_EVENT_ENTER,
        KeyCode::Esc => MITSURO_NATIVE_EVENT_ESC,
        KeyCode::Backspace => MITSURO_NATIVE_EVENT_BACKSPACE,
        KeyCode::Up => MITSURO_NATIVE_EVENT_UP,
        KeyCode::Down => MITSURO_NATIVE_EVENT_DOWN,
        KeyCode::Left => MITSURO_NATIVE_EVENT_LEFT,
        KeyCode::Right => MITSURO_NATIVE_EVENT_RIGHT,
        _ => return None,
    };

    Some(MitsuroNativeEvent {
        kind: MITSURO_NATIVE_EVENT_KEY,
        key_code,
...
mod tests {
    ...
    fn no_op_api() -> MitsuroNativePluginV1 {
        MitsuroNativePluginV1 {
    ...
    fn maps_basic_key_events() {
        ...
        assert_eq!(mapped.kind, MITSURO_NATIVE_EVENT_KEY);
    ...
    fn parses_special_keys() {
        ...
        let mapped = map_event(&event).expect("key event");
        assert_eq!(mapped.key_code, MITSURO_NATIVE_EVENT_ESC);
    }
    ...
    fn failed_native_load_removes_shadow_copy() {
        ...
        let shadow_dir = temp.path().join(".mitsuro-shadow");
