...
use super::retroarch_identity::GameBoyColorDirs;
...
static MITSURO_DIRS: std::sync::LazyLock<GameBoyColorDirs> =
    std::sync::LazyLock::new(GameBoyColorDirs::discover);
...
impl GameBoyRuntime {
    ...
    fn state_path(&self, slot: u8) -> Option<PathBuf> {
        ...
        Some(MITSURO_DIRS.states.join(filename))
    }
    ...
    fn load_state(&mut self, slot: u8) -> Result<(), String> {
        let core = self.core.as_ref().ok_or("No Game Boy core loaded")?;
        let canonical_path = self.state_path(slot).ok_or("No ROM loaded")?;
        let path = MITSURO_DIRS.read_path(&canonical_path);
    ...
    fn sram_path(&self) -> Option<PathBuf> {
        let rom_name = self.rom_path.as_ref()?.file_stem()?.to_string_lossy();
        Some(MITSURO_DIRS.saves.join(format!("{}.srm", rom_name)))
    }
    ...
    fn load_sram(&self) -> Result<(), String> {
        ...
        let core = self.core.as_ref().ok_or("No Game Boy core loaded")?;
        let canonical_path = self.sram_path().ok_or("No ROM loaded")?;
        let path = MITSURO_DIRS.read_path(&canonical_path);
...
impl RetroArchPlugin {
    pub fn new() -> Self {
        ...
        Self {
            ...
            current_rom_dir: MITSURO_DIRS.preferred_read_directory(&MITSURO_DIRS.roms),
            selected_core: None,
            menu_index: 0,
            scroll_offset: 0,
            pause_index: 0,
    ...
    fn scan_cores(&mut self) {
        ...
        for env_var in ["MITSURO_GAMEBOY_COLOR_CORE", "MITSURO_RETROARCH_CORE"] {
    ...
    fn core_search_dirs() -> Vec<PathBuf> {
        ...
        if let Ok(paths) = mitsuro_core::identity::env_var("MITSURO_GAMEBOY_COLOR_CORES") {
            dirs.extend(std::env::split_paths(&paths));
        }

        dirs.push(MITSURO_DIRS.cores.clone());
        if let Some(old_cores) = MITSURO_DIRS.legacy_read_directory(&MITSURO_DIRS.cores) {
            dirs.push(old_cores);
        }
        ...
        if let Ok(exe) = std::env::current_exe() {
            if let Some(exe_dir) = exe.parent() {
                ...
                dirs.push(exe_dir.join("../share/mitsuro/gameboy-color/cores"));
                dirs.push(exe_dir.join("../lib/mitsuro/gameboy-color/cores"));
            }
        }
...
impl Plugin for RetroArchPlugin {
    ...
    fn on_activate(&mut self) {
        ...
        let rom = mitsuro_core::identity::env_var("MITSURO_GAMEBOY_COLOR_ROM")
            .or_else(|_| mitsuro_core::identity::env_var("MITSURO_RETROARCH_ROM"));
