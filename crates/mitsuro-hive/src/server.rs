...
use mitsuro_hive_protocol::{
    ...
    DaemonStats, HiveEvent, IpcKey, NonceReplayGuard, PongResponse, ProtocolErrorPayload,
...
use crate::{HiveDaemonConfig, DAEMON_VERSION};
...
impl DaemonServer {
    pub async fn bind(config: HiveDaemonConfig, handler: Arc<dyn CommandHandler>) -> Result<Self> {
        if config.max_connections == 0 {
            bail!("Hive max_connections must be greater than zero");
        }
        let socket_parent = config
            .paths
            .socket_path
            .parent()
            .context("Hive socket path has no parent directory")?;
        mitsuro_hive_protocol::ensure_private_dir(socket_parent)
            .context("securing Hive socket directory")?;
        let key = Arc::new(
            IpcKey::load_or_create(&config.paths.key_path)
                .context("loading Hive IPC authentication key")?,
        );
    ...
    pub async fn serve(mut self) -> Result<()> {
        ...
        loop {
            tokio::select! {
                ...
                accepted = self.listener.accept() => {
                    let (stream, _) = accepted.context("accepting Hive IPC connection")?;
                    let permit = match Arc::clone(&self.connection_slots).try_acquire_owned() {
                        Ok(permit) => permit,
                        Err(_) => {
                            tracing::warn!(
                                max_connections = self.config.max_connections,
                                "Rejecting Hive IPC connection at the configured limit"
                            );
                            drop(stream);
                            continue;
                        }
                    ...
                    connections.spawn(async move {
                        ...
                        if let Err(error) = serve_connection(stream, connection, shutdown_rx).await {
                            tracing::warn!(error = %error, "Hive IPC connection ended with an error");
                        }
                    });
                }
                completed = connections.join_next(), if !connections.is_empty() => {
                    if let Some(Err(error)) = completed {
                        tracing::warn!(error = %error, "Hive IPC connection task panicked");
                    }
                }
            }
        }
        ...
        tracing::info!("Hive daemon stopping");
        ...
        .is_err()
        {
            tracing::warn!(?grace_period, "Aborting lingering Hive IPC connections");
    ...
    fn drop(&mut self) {
        ...
        if let Err(error) = remove_owned_socket(&path) {
            tracing::warn!(
                path = %path.display(),
                error = %error,
                "Failed to remove owned Hive socket during cleanup"
            );
        }
    }
}
...
struct ConnectionServices {
    ...
    metrics: Arc<ServerMetrics>,
    info: Arc<DaemonInfo>,
    auth_policy: mitsuro_hive_protocol::AuthPolicy,
...
async fn serve_connection(
    ...
    let peer = verify_same_user(&stream).context("rejecting cross-user Hive IPC peer")?;
                ...
                tokio::select! {
                        ...
                        match peer? {
                            ...
                            Some(frame) => {
                                tracing::warn!(
                                    ...
                                    "Closing Hive subscription after unexpected client input"
                                );
                                break;
                            }
                        }
                    ...
                    changed = shutdown_rx.changed() => {
                        if changed.is_err() || shutdown_rx.borrow().is_some() {
                            let event = mitsuro_hive_protocol::EventEnvelope {
                                version: ProtocolVersion::CURRENT,
                                session_id: None,
                                run_id: None,
                                sequence: None,
                                emitted_at_unix_ms: unix_time_millis(),
                                event: HiveEvent::DaemonShuttingDown {
...
async fn dispatch_request(
    ...
    peer: mitsuro_hive_protocol::PeerIdentity,
    services: &ConnectionServices,
...
async fn send_protocol_error(
    ...
    .await
    .context("sending Hive protocol error")
}
...
async fn read_client_frame(
    ...
    tokio::time::timeout(timeout, read_frame(stream))
        .await
        .with_context(|| format!("Hive IPC {phase} read timed out"))?
        .map_err(Into::into)
}
    ...
    W: AsyncWrite + Unpin,
{
    tokio::time::timeout(timeout, write_frame(stream, frame))
        .await
        .context("Hive IPC response write timed out")??;
    Ok(())
}
...
async fn bind_private_socket(path: &Path) -> Result<UnixListener> {
    if let Ok(metadata) = fs::symlink_metadata(path) {
        ...
        let expected_uid = mitsuro_hive_protocol::current_effective_uid();
        ...
        match tokio::time::timeout(Duration::from_millis(250), UnixStream::connect(path)).await {
            Ok(Ok(_)) => bail!("Hive daemon is already listening at {}", path.display()),
            Err(_) => bail!(
                "existing Hive socket {} did not respond; refusing destructive takeover",
                path.display()
            ),
            Ok(Err(error))
                if matches!(
            ...
            {
                fs::remove_file(path)
                    .with_context(|| format!("removing stale Hive socket {}", path.display()))?;
            }
            Ok(Err(error)) => {
                return Err(error)
                    .with_context(|| format!("checking existing Hive socket {}", path.display()));
            }
        }
    }

    let listener = UnixListener::bind(path)
        .with_context(|| format!("binding Hive socket {}", path.display()))?;
    if let Err(error) = fs::set_permissions(path, fs::Permissions::from_mode(0o600)) {
        drop(listener);
        let _ = remove_owned_socket(path);
        return Err(error).context("securing newly bound Hive socket");
    }
    Ok(listener)
}
...
fn secure_activated_socket(path: &Path) -> Result<()> {
    let metadata = fs::symlink_metadata(path)
        .with_context(|| format!("inspecting activated Hive socket {}", path.display()))?;
    if !metadata.file_type().is_socket() {
        bail!("activated Hive path {} is not a socket", path.display());
    }
    if metadata.uid() != mitsuro_hive_protocol::current_effective_uid() {
        bail!("activated Hive socket is owned by another user");
    }
...
fn remove_owned_socket(path: &Path) -> Result<()> {
    ...
    if !metadata.file_type().is_socket()
        || metadata.uid() != mitsuro_hive_protocol::current_effective_uid()
    {
        bail!(
    ...
    fs::remove_file(path).with_context(|| format!("removing Hive socket {}", path.display()))
}
...
fn activated_listener() -> Result<Option<UnixListener>> {
    ...
    // it before spawning any tool processes so children cannot keep the Hive
...
mod tests {
    ...
    use mitsuro_hive_protocol::{
        Actor, Command, DaemonRuntimeStats, ExtensionCommand, ExtensionResponse, HiveIpcClient,
        HiveIpcClientConfig, RequestEnvelope, ResponsePayload, ShutdownCommand, SubscribeCommand,
    ...
    fn test_config(temp: &tempfile::TempDir) -> HiveDaemonConfig {
        HiveDaemonConfig {
            paths: HivePaths {
                socket_path: temp.path().join("run").join("hive.sock"),
                key_path: temp.path().join("config").join("hive.key"),
            },
    ...
    async fn serves_ping_stats_extension_and_graceful_shutdown() {
        ...
        let client = HiveIpcClient::from_key_path(
            HiveIpcClientConfig::new(socket_path, "test-client"),
            key_path,
        )
        .unwrap();
        ...
        assert!(matches!(
            oversized_shutdown,
            mitsuro_hive_protocol::ClientError::Remote { ref code, .. }
    ...
    async fn rejects_a_client_with_the_wrong_key() {
        ...
        let client = HiveIpcClient::new(
            HiveIpcClientConfig::new(socket_path, "wrong-key"),
            IpcKey::generate(),
        );
        let error = client
        ...
        assert!(matches!(
            error,
            mitsuro_hive_protocol::ClientError::Remote { .. }
        ));
    ...
    async fn quiet_subscription_disconnect_releases_receiver_and_connection_slot() {
        ...
        let server_task = tokio::spawn(server.serve());
        let client = HiveIpcClient::from_key_path(
            HiveIpcClientConfig::new(socket_path, "quiet-subscription-test"),
            key_path,
        )
        .unwrap();
