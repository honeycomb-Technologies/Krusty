...
use mitsuro_core::hive::{canonical_timestamp, parse_utc_timestamp};
use mitsuro_core::storage::{hash_request_bytes, Database};
use mitsuro_hive_protocol::{
    unix_time_millis, Actor, DaemonRuntimeStats, EventEnvelope, ExtensionEvent, HiveEvent,
...
impl RuntimeStoreError {
    pub(crate) fn protocol(self) -> ProtocolErrorPayload {
        match self {
            ...
            Self::Internal(error) => {
                tracing::error!(error = %error, "durable Hive runtime storage failure");
                ProtocolErrorPayload::new(
                    "runtime_storage_error",
                    "durable Hive state could not be updated",
                    true,
                )
            }
        }
    ...
    connection
        .query_row(
            "SELECT id, session_id, status, timezone
             FROM hive_controllers WHERE session_id = ?1",
            [session_id],
            |row| {
                Ok(ControllerRecord {
                    id: row.get(0)?,
        ...
        .optional()?
        .ok_or_else(|| RuntimeStoreError::NotFound("Hive controller was not found".into()))
}
...
pub(crate) fn get_or_create_controller(
    ...
    let controller_id = crate::legacy_identity::controller_id(&session.id);
    connection.execute(
...
fn prune_controller_event_prefix(
    ...
    // Retention is prefix-only. Canonical run state lives in `hive_runs`, so
    ...
    let (protected_min, pending_interactions) = tx.query_row(
        ...
        WITH unresolved AS (
          SELECT e.sequence
            FROM hive_controller_events e
            JOIN hive_runs r ON r.id = e.run_id
           WHERE e.controller_id = ?1
             AND (
               ...
               AND NOT EXISTS (
                 SELECT 1
                   FROM hive_controller_events later
...
fn cleanup_expired_idempotency(tx: &Transaction<'_>, now: &str) -> Result<(), RuntimeStoreError> {
    tx.execute(
        "DELETE FROM hive_idempotency_keys
          WHERE rowid IN (
            SELECT rowid FROM hive_idempotency_keys
             WHERE expires_at <= ?1
...
fn cleanup_terminal_control_outbox(
    ...
    tx.execute(
        "DELETE FROM hive_control_outbox
          WHERE rowid IN (
            SELECT rowid FROM hive_control_outbox
