...
impl HiveRuntimeConfig {
    ...
    pub(crate) fn validate(&self) -> anyhow::Result<()> {
        ...
        anyhow::ensure!(
            ...
            "event channel capacity exceeds the bounded Hive runtime limits"
        );
        anyhow::ensure!(
            self.max_execution_event_bytes > 0
                && self.max_execution_event_bytes <= MAX_EXECUTION_EVENT_BYTES,
            "execution event byte limit is outside the bounded Hive runtime range"
        );
        ...
        anyhow::ensure!(
            ...
            "event buffer byte budget exceeds the bounded Hive runtime limit"
        );
...
mod tests {
    ...
    use super::{
        HiveRuntimeConfig, MAX_EXECUTION_EVENT_BYTES, MAX_EXECUTION_EVENT_CAPACITY,
        MAX_LIVE_EVENT_CAPACITY, MAX_SUBSCRIBER_CAPACITY,
    };

    #[test]
    fn scheduler_must_renew_before_its_daemon_lease_expires() {
        let mut config = HiveRuntimeConfig::for_database("runtime.db");
    ...
    fn default_event_buffers_stay_within_a_small_bounded_budget() {
        let config = HiveRuntimeConfig::for_database("runtime.db");
