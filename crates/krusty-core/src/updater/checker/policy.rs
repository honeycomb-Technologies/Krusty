...
#[cfg(unix)]
const UNIX_MANAGED_UPDATE_MESSAGE: &str = "Automatic updates are disabled on Unix because krusty, krusty-mako, and their service units must be upgraded together. Upgrade the complete Krusty release with your package manager (Homebrew/AUR), or rerun the checksum-verifying installer from https://github.com/honeycomb-Technologies/Krusty/blob/main/install.sh.";
