...
impl SkillsManager {
    ...
    ///
    /// When the project root has the conventional `.krusty/skills` shape, the
    ...
    /// Create a manager with Krusty, Agent Skills, OpenCode, Claude, Codex, and
    ...
    fn with_discovered_roots(working_dir: &Path, native_global: Option<PathBuf>) -> Self {
        ...
        let global_dir = native_global.unwrap_or_else(|| home.join(".krusty/skills"));
        let mut roots = vec![
            SkillRoot::new(
        ...
        policy_files.extend(
            ancestors
                .iter()
                .map(|directory| directory.join(".krusty").join(POLICY_FILE_NAME)),
        );
        ...
        let project_dir = nearest.join(".krusty/skills");
        let policy_write_path = nearest.join(".krusty").join(POLICY_FILE_NAME);
    ...
    pub fn delete_skill(&mut self, name: &str) -> Result<()> {
        ...
        if skill.source != SkillSource::Global || !skill.path.starts_with(&self.global_dir) {
            return Err(anyhow!(
                "Only native ~/.krusty global skills can be deleted; compatible, project, and package skills are managed at their source"
            ));
        }
...
fn infer_working_dir(project_skills: &Path) -> Option<&Path> {
    ...
    if state.file_name().and_then(|name| name.to_str()) != Some(".krusty") {
        return None;
    }
    state.parent()
}
...
mod tests {
    ...
    fn project_policy_can_narrow_but_not_loosen_global_policy() {
        ...
        let root_policy = temp.path().join("repo/.krusty/skills-policy.json");
        let nearest_policy = temp.path().join("repo/nested/.krusty/skills-policy.json");
        write_policy_file(
            &global_policy,
            &SkillPolicyFile {
