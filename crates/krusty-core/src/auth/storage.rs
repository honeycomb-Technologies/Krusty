...
impl OAuthTokenStore {
    ...
    fn save_to_disk(&self, path: &Path) -> Result<()> {
        ...
        // A process-specific temporary path prevents unrelated Krusty
    ...
    /// Serialize one provider's full refresh exchange across Krusty processes.
