...
impl AgentExtensionManager {
    ...
    pub fn new_with_paths(
        ...
        let roots = vec![
            ...
            AgentExtensionRoot::new(
                working_dir.join(".krusty").join("extensions"),
    ...
    /// own `.krusty/settings.json` can narrow an existing grant but can never
                ...
                || !project_settings.allows_agent_extension(&extension_id)
            {
                ...
                if candidate.manifest.enabled {
                    self.record_diagnostic(
                        ...
                        Some(extension_id),
                        "info",
                        "Agent extension disabled by .krusty/settings.json policy".to_string(),
                    )
                    .await;
                }
                continue;
...
async fn discover_root(
    ...
    if root.path.join("krusty-extension.json").is_file() {
        return match discover_path(root, &root.path).await {
            Ok(Some(candidate)) => Ok((vec![candidate], Vec::new())),
            Ok(None) => Ok((Vec::new(), Vec::new())),
            Err(error) => Ok((
...
async fn discover_path(
    ...
    if path.is_dir() {
        let manifest_path = path.join("krusty-extension.json");
...
mod tests {
    ...
    async fn discovers_manifest_and_standalone_extensions() {
        ...
        fs::write(
            root.join("manifested/krusty-extension.json"),
    ...
    async fn bun_extension_registers_tools_commands_events_and_context() {
        ...
        let extension_dir = working_dir.join(".krusty").join("extensions").join("smoke");
...
export default (krusty) => {
  ...
  krusty.registerCommand("hello-extension", {
    description: "Return a command greeting",
    handler: (argument) => `hello ${argument}`
  });
  krusty.on("turn_complete", () => { events += 1; });
  ...
  krusty.addContext(() => "Context supplied by the smoke extension.");
};
"#,
        )
        .expect("entry");
        ...
        fs::write(extension_dir.join("krusty-extension.json"), "{")
    ...
    async fn repository_settings_cannot_self_authorize_project_code() {
        ...
        .expect("entry");
        fs::write(
            working_dir.join(".krusty/settings.json"),
