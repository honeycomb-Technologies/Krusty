...
fn seed_session_from_store(store: &MakoRuntimeStateStore, session_id: &str, title: &str) {
    let now = chrono::Utc::now().to_rfc3339();
    store
        .conn()
        .execute(
...
fn list_recoverable_states_only_returns_running_and_sleeping() {
    let (store, _tmp) = create_store();
    seed_session_from_store(&store, "sess-2", "Mako Sleep");
    store
        .set_state(
            "sess-1",
            MakoRuntimeStateStatus::Running,
    ...
    assert!(states.iter().any(
        |state| state.session_id == "sess-1" && state.status == MakoRuntimeStateStatus::Running
    ));
    assert!(states
        .iter()
...
fn list_states_for_sessions_returns_requested_rows_only() {
    let (store, _tmp) = create_store();
    seed_session_from_store(&store, "sess-2", "Mako Sleep");
    seed_session_from_store(&store, "sess-3", "Mako Idle");
    store
        .set_state(
            "sess-1",
            MakoRuntimeStateStatus::Running,
