...
#[derive(Debug, Clone)]
pub struct MakoAttentionItemState {
    pub item_id: String,
    pub read: bool,
    pub cleared: bool,
...
pub struct MakoAttentionStateStore<'a> {
    db: &'a Database,
}

impl<'a> MakoAttentionStateStore<'a> {
    ...
    pub fn list_for_user(
        &self,
        user_id: Option<&str>,
    ) -> Result<HashMap<String, MakoAttentionItemState>> {
        let mut stmt = self.db.conn().prepare(
            "SELECT item_id, read, cleared, updated_at
             FROM mako_attention_state
             WHERE user_scope = ?1",
        )?;

        let rows = stmt.query_map([user_scope(user_id)], |row| {
            Ok(MakoAttentionItemState {
                item_id: row.get(0)?,
    ...
    fn upsert_state(
        ...
        self.db.conn().execute(
            "INSERT INTO mako_attention_state (user_scope, item_id, read, cleared, updated_at)
             VALUES (?1, ?2, ?3, ?4, ?5)
             ON CONFLICT(user_scope, item_id) DO UPDATE SET
                read = COALESCE(?6, mako_attention_state.read),
                cleared = COALESCE(?7, mako_attention_state.cleared),
...
mod tests {
    ...
    use super::MakoAttentionStateStore;
