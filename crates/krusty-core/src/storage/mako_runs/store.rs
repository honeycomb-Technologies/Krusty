...
impl MakoRunStore {
    ...
    /// delivered). The caller's `ClaimedMakoRun` is an immutable snapshot;
    ...
    pub fn validate_claimed_execution_fenced(
        ...
        let row = self
            ...
            .query_row(
                "SELECT r.objective, r.config_json, r.kind,
                 ...
                 JOIN mako_run_attempts a ON a.id = ?7
                 JOIN mako_daemon_leases d ON d.lease_name = ?8
                 ...
                 WHERE r.id = ?1 AND r.controller_id = ?2 AND r.session_id = ?3
                   ...
                   AND c.user_id IS s.user_id AND s.session_type = 'mako'
            ...
            .optional()
            .context("validating claimed Mako execution fence")?;
        ...
        let persisted_config: serde_json::Value = serde_json::from_str(&config_json)
            .context("decoding claimed Mako run config during fence validation")?;
        ...
        if matches!(kind.as_str(), "scheduled" | "controller_child") {
            ...
            let materialized = message_content
                ...
                .transpose()
                .context("decoding materialized Mako objective message")?;
    ...
    pub fn list_for_controller(&self, controller_id: &str, limit: usize) -> Result<Vec<MakoRun>> {
        if limit == 0 {
            return Ok(Vec::new());
        }
        let sql = format!(
            "SELECT {RUN_COLUMNS} FROM mako_runs
        ...
        let rows = statement
            ...
            .context("listing Mako runs for controller")?;
        Ok(rows)
    }
            ...
            .query_row(
                "SELECT r.id
                 ...
                 WHERE r.status = 'queued'
                   ...
                   AND NOT EXISTS (
                       SELECT 1 FROM mako_runs uncertain
    ...
    fn mark_running_inner(
        ...
        let changed = tx.execute(
            "UPDATE mako_runs
             ...
             WHERE id = ?1 AND lease_token = ?2 AND lease_epoch = ?3
               ...
               AND EXISTS (
                   SELECT 1 FROM mako_controllers c
                   WHERE c.id = mako_runs.controller_id AND c.status = 'active'
               )",
        ...
        if changed == 1 {
            ...
            update_derived_state(&tx, run_id, MakoRunStatus::Running, &now)?;
        }
        tx.commit()?;
        Ok(changed == 1)
    }
    ...
    pub fn finish_cancelled_claim_fenced(
        ...
        anyhow::ensure!(
            completion.target_status == MakoRunStatus::Cancelled,
    ...
    fn finish_claimed_inner(
            ...
            .query_row(
                "SELECT r.status, r.attempt_count, r.max_attempts
                 ...
                 WHERE r.id = ?1 AND r.lease_token = ?2 AND r.lease_epoch = ?3
                   ...
                   AND EXISTS (
                       SELECT 1 FROM mako_run_attempts a
        ...
        anyhow::ensure!(
            attempt_changed == 1,
            "claimed Mako run has no matching open attempt during completion"
        );
    ...
    fn reconcile_expired_leases_inner(
        ...
        for (run_id, status, attempt_no, lease_token) in expired {
            ...
            update_derived_state(&tx, &run_id, target, &now)?;
            if target == MakoRunStatus::RecoveryRequired {
    ...
    fn promote_due_runs_inner(
        ...
        for run in &run_ids {
            update_derived_state(&tx, &run.run_id, MakoRunStatus::Queued, &now)?;
        }
        tx.commit()?;
        Ok(run_ids)
    }
    ...
    pub fn cancel(&self, run_id: &str, now: DateTime<Utc>, reason: &str) -> Result<bool> {
        ...
        if changed == 1 {
            ...
            update_derived_state(&tx, run_id, MakoRunStatus::Cancelled, &now)?;
    ...
    pub fn requeue(&self, run_id: &str, now: DateTime<Utc>) -> Result<bool> {
        ...
        if changed == 1 {
            update_derived_state(&tx, run_id, MakoRunStatus::Queued, &now)?;
        }
        tx.commit()?;
        Ok(changed == 1)
    }
...
fn update_derived_state(
    ...
    if status == MakoRunStatus::RecoveryRequired {
        tx.execute(
    ...
    if let Some(occurrence_id) = occurrence_id {
        let occurrence_status = match status {
            MakoRunStatus::Queued | MakoRunStatus::Leased => "queued",
            MakoRunStatus::Succeeded => "succeeded",
            MakoRunStatus::Cancelled => "cancelled",
            MakoRunStatus::Failed | MakoRunStatus::DeadLetter | MakoRunStatus::RecoveryRequired => {
                "failed"
            }
            MakoRunStatus::Running
            | MakoRunStatus::Sleeping
            | MakoRunStatus::RetryWait
            | MakoRunStatus::AwaitingInput => "running",
        };
        tx.execute(
...
fn materialize_objective_message(tx: &Transaction<'_>, run_id: &str, now: &str) -> Result<()> {
    let objective = tx
        .query_row(
            "SELECT session_id, kind, objective FROM mako_runs
    ...
    tx.execute(
        "UPDATE mako_runs SET objective_message_id = ?2 WHERE id = ?1",
