...
fn list_for_user_is_scoped_by_controller_owner() {
    ...
    db.conn()
        .execute_batch(
            "INSERT INTO sessions (id, title, created_at, updated_at, session_type)
             VALUES
               ('session-a', 'A', '2026-07-01T00:00:00.000000Z', '2026-07-01T00:00:00.000000Z', 'mako'),
               ('session-b', 'B', '2026-07-01T00:00:00.000000Z', '2026-07-01T00:00:00.000000Z', 'mako'),
               ('session-c', 'C', '2026-07-01T00:00:00.000000Z', '2026-07-01T00:00:00.000000Z', 'mako');
             INSERT INTO mako_controllers (
                 id, scope_key, user_id, session_id, status, timezone,
                 max_concurrent_runs, created_at, updated_at
             ) VALUES
        ...
        .unwrap();
    let store = MakoScheduleStore::new(db);
