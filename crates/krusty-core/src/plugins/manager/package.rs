...
impl PluginManager {
    ...
    async fn ensure_npm_project(&self, install_root: &Path) -> Result<()> {
        ...
        let content = serde_json::json!({
            "name": "krusty-plugin-transaction",
            "private": true
        });
    ...
    async fn discover_package_plugin_manifests(&self, package_root: &Path) -> Result<Vec<PathBuf>> {
        if let Some(package_json) = self.read_package_json(package_root).await? {
            if let Some(krusty) = package_json.krusty {
                if krusty.plugins.len() > MAX_PACKAGE_PLUGIN_MANIFESTS {
                    bail!(
                        "package.json krusty.plugins declares {} manifests; the limit is {}",
                        krusty.plugins.len(),
                        MAX_PACKAGE_PLUGIN_MANIFESTS
                    );
                }
                let mut manifests = Vec::with_capacity(krusty.plugins.len());
                let mut seen = BTreeSet::new();
                for (index, plugin_path) in krusty.plugins.into_iter().enumerate() {
                    let rel = normalize_package_manifest_path(
                        &plugin_path,
                        &format!("package.json krusty.plugins[{}]", index),
                    )?;
                    if !seen.insert(rel.clone()) {
                        bail!(
                            "package.json krusty.plugins contains duplicate manifest path '{}'",
                            rel.display()
                        );
                    }
                    manifests.push(rel);
    ...
    if cfg!(unix) {
        ...
        bail!(
            "local and npm plugin package installation is unavailable on this platform because \
             Krusty cannot enforce its no-follow, stable-identity, and hard-link snapshot \
