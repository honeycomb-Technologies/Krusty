...
/// Inputs captured by the Mako runtime when a canonical turn completes.
...
impl LearningReviewModel for AiLearningReviewModel {
    async fn review(&self, system_prompt: &str, user_prompt: &str) -> Result<SimpleCallResult> {
        ...
        self.trace
            .record_simple_call(
                "mako_post_turn_learning_review",
...
pub async fn review_latest_completed_mako_turn(
...
async fn review_latest_with_model(
    ...
    match (review_result, finish_result) {
        ...
        (Ok(_), Err(error)) => Err(error).context("finish Mako learning checkpoint"),
        ...
        (Err(error), Err(finish_error)) => {
            tracing::warn!(
                ...
                through_message_id,
                error = %finish_error,
                "Failed to mark Mako learning review checkpoint failed"
            );
            Err(error)
        }
    }
...
fn prepare_review(
    ...
    if session.session_type != SessionType::Mako {
        bail!("post-turn learning is restricted to Mako sessions");
    }
...
async fn execute_claimed_review(
    ...
    let transcript_json = prepared
        .transcript
        .prompt_json()
        .context("encode bounded Mako learning transcript")?;
    let prompt = format!(
...
mod tests {
    ...
    #[test]
    fn reviewer_does_not_accept_non_mako_sessions() {
        let temp = TempDir::new().unwrap();
        let db_path = temp.path().join("not-mako.db");
        let db = Database::new(&db_path).unwrap();
        db.conn()
            .execute(
                "INSERT INTO users (id, email, license_tier) VALUES ('alice', 'alice@learning.test', 'free')",
