...
use krusty_core::storage::{
    ClaimRunRequest, DaemonFence, DaemonLeaseAcquire, Database, MakoDaemonLeaseStore, MakoRunStore,
    MakoScheduleStore, RunCompletion, SessionManager, SessionType, WorkspaceMode,
};
...
use super::{
    ...
    ExecutionOutcome, ExecutionRequest, MakoRuntimeConfig,
};
...
impl ExecutionBackend for PrivacyBoundaryBackend {
    async fn execute(&self, request: ExecutionRequest) -> ExecutionOutcome {
        const SENTINEL: &str = "MAKO_PRIVATE_SENTINEL_91f6";
        for payload in [
            serde_json::json!({
...
async fn wait_for_runtime_health(
    handler: &DurableMakoCommandHandler,
    pump_alive: bool,
...
async fn foreign_scheduler_lease_keeps_runtime_not_ready_until_takeover() {
    ...
    let lease =
        match MakoDaemonLeaseStore::new(Database::new(&runtime_config.database_path).unwrap())
            .acquire(
                "mako-scheduler",
                "foreign-daemon",
                chrono::Utc::now(),
    ...
    assert!(
        MakoDaemonLeaseStore::new(Database::new(&runtime_config.database_path).unwrap())
            .release("mako-scheduler", "foreign-daemon", lease.fencing_token,)
            .unwrap()
    );
...
async fn generic_wake_is_ambiguous_but_user_response_resumes_only_its_exact_run() {
    ...
    let (controller_id, run_a): (String, String) = db
        .conn()
        .query_row(
            "SELECT controller_id, id FROM mako_runs WHERE session_id = ?1",
            [&session_id],
    ...
    db.conn()
        .execute(
            "UPDATE mako_runs SET status = 'awaiting_input', updated_at = ?2 WHERE id = ?1",
    ...
    for (run_id, tool_call_id) in [(&run_a, "question-a"), (&run_b, "question-b")] {
        let sequence = db
            .conn()
            .query_row(
                "SELECT COALESCE(MAX(sequence), 0) + 1 FROM mako_controller_events
    ...
    assert_eq!(
        db.conn()
            .query_row(
                "SELECT status FROM mako_runs WHERE id = 'run-b'",
                [],
                |row| { row.get::<_, String>(0) }
            )
            .unwrap(),
    ...
    let event_leaks: i64 = db
        .conn()
        .query_row(
            "SELECT COUNT(*) FROM mako_controller_events
    ...
    MakoDaemonLeaseStore::new(Database::new(&runtime_config.database_path).unwrap())
        .release("mako-scheduler", "foreign-daemon", foreign.fencing_token)
        .unwrap();
    runtime.shutdown().await;
}
...
async fn graceful_runtime_shutdown_releases_lease_without_supervision_failure() {
    ...
    let lease = MakoDaemonLeaseStore::new(Database::new(&runtime_config.database_path).unwrap())
        .get("mako-scheduler")
        .unwrap()
...
async fn recurring_schedule_inherits_frozen_session_config_and_rejects_stale_revision() {
    ...
    Database::new(&runtime_config.database_path)
        ...
        .conn()
        .execute(
            "UPDATE mako_runtime_state SET crew_slug = 'ops' WHERE session_id = ?1",
            [&session_id],
        )
        .unwrap();
...
async fn every_session_command_reloads_and_enforces_exact_actor_ownership() {
    ...
    assert_eq!(
        Database::new(&config(&temp).database_path)
            ...
            .conn()
            .query_row(
                "SELECT COUNT(*) FROM mako_controllers WHERE session_id = ?1",
                [&code_session_id],
                |row| row.get::<_, i64>(0),
            )
            .unwrap(),
...
async fn dropped_producer_after_tool_event_is_recovery_required_and_unclaimable() {
    ...
    let claim = MakoRunStore::new(Database::new(&runtime_config.database_path).unwrap())
...
async fn restart_marks_expired_running_work_recovery_required_without_replay() {
    ...
    db.conn()
        .execute_batch(&format!(
            "INSERT INTO sessions (id, title, created_at, updated_at, model, session_type)
             VALUES ('session-1', 'Mako', '{now}', '{now}', 'test:model', 'mako');
    ...
    let (runs, run_status, runtime_status, controller_status): (i64, String, String, String) = db
        ...
        .query_row(
            "SELECT
                 (SELECT COUNT(*) FROM mako_runs WHERE session_id = 'session-1'),
                 (SELECT status FROM mako_runs WHERE id = 'run-1'),
                 (SELECT status FROM mako_runtime_state WHERE session_id = 'session-1'),
                 (SELECT status FROM mako_controllers WHERE id = 'controller-1')",
            [],
...
async fn user_response_at_awaiting_boundary_is_not_stranded() {
    ...
    wait_for(|| {
        ...
        db.conn()
            .query_row(
                "SELECT COUNT(*) FROM mako_controller_events e
                 JOIN mako_controllers c ON c.id = e.controller_id
    ...
    let run_id = Database::new(&runtime_config.database_path)
        ...
        .conn()
        .query_row(
            "SELECT id FROM mako_runs WHERE session_id = ?1 ORDER BY created_at LIMIT 1",
            [&session_id],
            |row| row.get::<_, String>(0),
        )
        .unwrap();
    ...
    wait_for(|| {
        ...
        db.conn()
            .query_row(
                "SELECT COUNT(*) FROM mako_runs WHERE session_id = ?1 AND status = 'succeeded'",
                [&session_id],
                |row| row.get::<_, i64>(0),
            )
            .unwrap()
    ...
    let (rows, status, attempts): (i64, String, i64) = db
        ...
        .query_row(
            "SELECT COUNT(*), MAX(status), MAX(attempt_count)
             FROM mako_control_outbox WHERE session_id = ?1 AND dedupe_key = ?2",
            rusqlite::params![session_id, format!("{run_id}:tool-1")],
            |row| Ok((row.get(0)?, row.get(1)?, row.get(2)?)),
        )
        .unwrap();
    ...
    let (run_id, lease_token, attempt_outcome, finished_at): (
        ...
        .query_row(
            "SELECT r.id, r.lease_token, a.outcome, a.finished_at
             FROM mako_runs r JOIN mako_run_attempts a ON a.run_id = r.id
...
async fn active_cancel_persists_terminal_event_then_quiesces_for_delete() {
    ...
    let (open_attempts, cancelled_attempts, finish_events): (i64, i64, i64) = db
        ...
        .query_row(
            "SELECT
                 (SELECT COUNT(*) FROM mako_run_attempts a
                  JOIN mako_runs r ON r.id = a.run_id
    ...
    let outcome_kind = db
        ...
        .query_row(
            "SELECT json_extract(outcome_json, '$.kind')
             FROM mako_runs WHERE session_id = ?1",
            [&session_id],
            |row| row.get::<_, String>(0),
        )
        .unwrap();
...
async fn leased_cancel_closes_unstarted_attempt_immediately() {
    ...
    let claimed = MakoRunStore::new(Database::new(&runtime_config.database_path).unwrap())
        .claim_next(&ClaimRunRequest {
...
async fn noncooperative_cancel_terminalizes_after_grace_and_rejects_late_worker_writes() {
    ...
    let (run_id, lease_token, lease_epoch): (String, String, u64) = {
        ...
        db.conn()
            .query_row(
                "SELECT id, lease_token, lease_epoch FROM mako_runs
    ...
    wait_for(|| {
        ...
        db.conn()
            .query_row(
                "SELECT
                     EXISTS(SELECT 1 FROM mako_runs
                            WHERE id = ?1 AND status = 'cancelled'),
                     EXISTS(SELECT 1 FROM mako_controller_events
    ...
    let attempt: (String, Option<String>, String, String) = db
        ...
        .query_row(
            "SELECT outcome, finished_at, stop_reason, error
             FROM mako_run_attempts WHERE run_id = ?1",
            [&run_id],
            |row| Ok((row.get(0)?, row.get(1)?, row.get(2)?, row.get(3)?)),
        )
        .unwrap();
    ...
    let runtime_projection: (String, String) = db
        .conn()
        .query_row(
            "SELECT status, current_run_id FROM mako_runtime_state WHERE session_id = ?1",
            [&session_id],
            |row| Ok((row.get(0)?, row.get(1)?)),
        )
        .unwrap();
    ...
    let store = MakoRunStore::new(Database::new(&runtime_config.database_path).unwrap());
    let late_success = RunCompletion {
        target_status: MakoRunStatus::Succeeded,
        now: chrono::Utc::now(),
        available_at: None,
        wake_at: None,
...
async fn server_created_session_gets_controller_and_waiting_runs_resume_durably() {
    ...
    db.conn()
        .execute(
            "INSERT INTO sessions (
             ...
             ) VALUES ('legacy-session', 'Legacy', ?1, ?1, '/work', '/work', 'test:model', 'mako')",
            [&now],
        )
        .unwrap();
    db.conn()
    ...
    let (run_id, controller_id): (String, String) = db
        .conn()
        .query_row(
            "SELECT id, controller_id FROM mako_runs WHERE session_id = 'legacy-session'",
            [],
            |row| Ok((row.get(0)?, row.get(1)?)),
        )
        .unwrap();
...
async fn live_only_unsequenced_events_are_forwarded_without_advancing_cursor() {
    ...
    assert!(matches!(
        event.event,
        MakoEvent::Runtime(runtime) if runtime.event_type == "live_delta"
    ));
    runtime.shutdown().await;
}
...
async fn backend_private_error_output_and_tool_payload_never_enter_durable_state() {
    ...
    wait_for(|| {
        Database::new(&runtime_config.database_path)
            ...
            .conn()
            .query_row(
                "SELECT COUNT(*) FROM mako_runs WHERE status IN ('failed', 'succeeded')",
                [],
                |row| row.get::<_, i64>(0),
            )
            .unwrap()
    ...
    let leaked: i64 = db
        ...
        .query_row(
            "SELECT
               ...
               + (SELECT COUNT(*) FROM mako_run_attempts
    ...
    db.conn()
        .execute(
            "INSERT INTO sessions (
             ...
             ) VALUES (?1, 'Mako', ?2, ?2, 'test:model', 'mako')",
...
async fn durable_runtime_stats_count_each_scheduler_state_exactly() {
    ...
    db.conn()
        .execute(
            "UPDATE mako_controllers SET status = 'paused' WHERE id = 'controller-inactive'",
            [],
        )
        .unwrap();
    db.conn()
...
async fn unresolved_interaction_spam_is_rejected_without_losing_pending_truth() {
    ...
    assert_eq!(
        db.conn()
            .query_row(
                "SELECT COUNT(*) FROM mako_controller_events WHERE controller_id = ?1",
                [&controller.id],
                |row| row.get::<_, i64>(0),
            )
            .unwrap(),
    ...
    assert_eq!(
        db.conn()
            .query_row(
                "SELECT MIN(sequence) FROM mako_controller_events WHERE controller_id = ?1",
                [&controller.id],
                |row| row.get::<_, i64>(0),
            )
            .unwrap(),
...
fn dedupe_hit_runs_retention_and_terminal_outbox_maintenance() {
    ...
    assert_eq!(
        db.conn()
            .query_row(
                "SELECT COUNT(*) FROM mako_control_outbox WHERE id = 'outbox-old'",
                [],
                |row| row.get::<_, i64>(0),
            )
            .unwrap(),
