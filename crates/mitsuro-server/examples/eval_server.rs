...
use mitsuro_server::ServerConfig;
...
async fn main() -> Result<()> {
    let port = std::env::var("MITSURO_EVAL_PORT")
        .unwrap_or_else(|_| "3100".to_string())
        .parse::<u16>()
        .context("MITSURO_EVAL_PORT must be a valid TCP port")?;
    let working_dir = required_path("MITSURO_EVAL_WORKING_DIR")?;
    let database_path = required_path("MITSURO_EVAL_DATABASE_PATH")?;
    ...
    mitsuro_server::start_isolated_server_with_listener(config, listener).await
}
