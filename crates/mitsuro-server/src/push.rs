...
impl PushService {
    ...
    pub fn init(vapid_key_path: &std::path::Path, db_path: Arc<PathBuf>) -> Result<Self> {
        ...
        let contact = mitsuro_core::identity::env_var("MITSURO_PUSH_CONTACT")
            .ok()
            .filter(|v| !v.trim().is_empty())
            .unwrap_or_else(|| "mailto:mitsuro@localhost".to_string());
    ...
    async fn send_expo_once(
        ...
        let ttl = match event_type {
            ...
            PushEventType::HiveUpdate => 24 * 60 * 60,
        ...
        let mut request = self
            ...
            .json(&message);
        if let Ok(access_token) = mitsuro_core::identity::env_var("MITSURO_EXPO_PUSH_ACCESS_TOKEN")
        {
    ...
    pub async fn notify_user(
        ...
        let delivery_payload = bridge_delivery_payload(&payload);
        ...
        for sub in subscriptions {
            ...
            let outcome = self.send_with_retry(&sub, &delivery_payload).await;
            match &outcome {
        ...
        for device in expo_devices {
            ...
            let response = self
                .send_expo_once(&device, &delivery_payload, event_type)
                .await;
            match response {
                Ok(response) => {
...
fn bridge_delivery_payload(payload: &PushPayload) -> PushPayload {
    let mut delivery = payload.clone();
    crate::legacy_identity::bridge_hive_notification(&mut delivery.category, &mut delivery.data);
    delivery
}
...
fn record_expo_attempt(
    ...
    let endpoint = format!(
        ...
        &mitsuro_core::storage::hash_request_bytes(device.expo_push_token.as_bytes())[..12],
    );
...
mod tests {
    ...
    use super::{bridge_delivery_payload, PushEventType, PushPayload, PushService};

    #[test]
    fn legacy_hive_event_is_read_but_canonical_value_is_emitted() {
        let parsed = PushEventType::from_str(crate::legacy_identity::HIVE_PUSH_EVENT).unwrap();
        assert_eq!(parsed.as_str(), "hive_update");
    }

    #[test]
    fn hive_push_bridge_changes_delivery_copy_not_canonical_intent_payload() {
        let canonical = PushPayload {
            title: "Hive".into(),
            body: "Updated".into(),
        ...
        let delivery = bridge_delivery_payload(&canonical);

        assert_eq!(
            canonical.data.as_ref().expect("canonical data")["type"],
            "hive_update"
        );
        assert_eq!(delivery.category.as_deref(), Some("MAKO_SESSION"));
        assert_eq!(
            delivery.data.as_ref().expect("delivery data")["type"],
            "mako_update"
        );
        assert_eq!(
            delivery.data.as_ref().expect("delivery data")["focus"],
            "mako"
        );
    }
    ...
    fn concurrent_vapid_initializers_converge_on_one_private_key() {
        ...
        let db_path = Arc::new(temp.path().join("mitsuro.db"));
