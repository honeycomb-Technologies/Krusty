...
async fn register_device(
    ...
    let bundle_id = registration_bundle_id(req.bundle_id.as_deref());
...
fn registration_bundle_id(requested: Option<&str>) -> &str {
    requested.unwrap_or(crate::legacy_identity::APNS_BUNDLE_ID)
}
...
#[cfg(test)]
mod tests {
    use super::{registration_bundle_id, validate_content_state, validate_live_activity_token};

    #[test]
    fn omitted_registration_bundle_keeps_deprecated_install_topic_during_bridge() {
        assert_eq!(
            registration_bundle_id(None),
            crate::legacy_identity::APNS_BUNDLE_ID
        );
        assert_eq!(
            registration_bundle_id(Some(crate::apns::DEFAULT_APNS_BUNDLE_ID)),
            crate::apns::DEFAULT_APNS_BUNDLE_ID
        );
    }
