...
use mitsuro_core::agent::PackageHookConfig;
use mitsuro_core::mcp::{McpConnectionAuthority, McpPackageConfig};
use mitsuro_core::plugins::{
...
pub(crate) fn plugin_manager() -> PluginManager {
    ...
    MANAGER
        .get_or_init(|| {
            PluginManager::new(reqwest::Client::new(), mitsuro_core::paths::plugins_dir())
        })
        .clone()
}
...
async fn refresh_plugin_contributions(
    ...
    match state.mcp_manager.load_config().await {
        Ok(()) => {
            ...
            state.tool_registry.unregister_by_prefix("mcp__").await;
            mitsuro_core::mcp::tool::register_mcp_tools(
...
mod tests {
    ...
    use crate::error::AppError;
    use mitsuro_core::plugins::PluginPermissionSet;
