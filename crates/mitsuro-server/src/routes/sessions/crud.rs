...
use mitsuro_core::ai::models::{ModelKey, ModelLookupError, ModelMetadata};
use mitsuro_core::storage::{SessionType, WorkspaceMode};
...
async fn resolve_requested_model_selection(
    ...
    match state.model_registry.resolve_legacy_key(model_id).await {
        ...
        // explicit MITSURO_PROVIDER at execution time.
...
pub(super) async fn list_sessions(
    ...
    let response: Vec<SessionResponse> = sessions
        .into_iter()
        .map(|session| SessionResponse::from_session(session, wire_format))
        .collect();
...
pub(super) async fn create_session(
    ...
    if session_type == SessionType::Hive {
        return Err(AppError::Conflict(
            "Hive sessions must be created through POST /hive/dispatch so the background service owns the durable controller".into(),
        ));
    }
    let session_manager = open_session_manager(&state)?;
    let workspace_scope = request_workspace_scope(&state, user.as_ref());
