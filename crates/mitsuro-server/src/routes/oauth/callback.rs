...
fn callback_page(
    ...
    let legacy_payload = json!({
        "type": crate::legacy_identity::OAUTH_COMPLETE_EVENT_TYPE,
        "provider": &provider,
        "success": success,
        "error": &error,
        "issued_at": issued_at,
    ...
    let legacy_payload_json = script_safe_json(
...
<body>
  ...
  <script>
    ...
    const legacyPayload = {legacy_payload_json};
    ...
    try {{
      if (window.opener && !window.opener.closed) {{
        ...
        window.opener.postMessage(legacyPayload, window.location.origin);
        window.opener.focus();
      }}
    }} catch (error) {{
    ...
    try {{
      if ('BroadcastChannel' in window) {{
        ...
        channel.close();
        const legacyChannel = new BroadcastChannel({legacy_channel_name:?});
        legacyChannel.postMessage(legacyPayload);
        legacyChannel.close();
      }}
    }} catch (error) {{
    ...
    setTimeout(() => {{
      if (window.location.pathname !== returnUrl) {{
        ...
        legacy_storage_key = crate::legacy_identity::OAUTH_RESULT_STORAGE_KEY,
        channel_name = OAUTH_RESULT_CHANNEL,
        legacy_channel_name = crate::legacy_identity::OAUTH_RESULT_CHANNEL,
    );
...
mod tests {
    ...
    async fn callback_page_does_not_embed_raw_script_end_tag_in_payload() -> anyhow::Result<()> {
        let response = callback_error_page(
            "openai".to_string(),
            "</script><script>window.__mitsuro_xss=1</script> (access_denied)".to_string(),
        );
        let bytes = body::to_bytes(response.into_body(), usize::MAX).await?;
        let html = String::from_utf8(bytes.to_vec())?;

        assert!(html.contains("&lt;/script&gt;&lt;script&gt;window.__mitsuro_xss=1&lt;/script&gt;"));
        assert!(html.contains(
            r#"\u003c/script\u003e\u003cscript\u003ewindow.__mitsuro_xss=1\u003c/script\u003e"#
        ));
    ...
    #[tokio::test]
    async fn callback_page_signals_canonical_and_deprecated_browser_clients() -> anyhow::Result<()>
    {
        let response = callback_success_page("openai".to_string());
        let bytes = body::to_bytes(response.into_body(), usize::MAX).await?;
        let html = String::from_utf8(bytes.to_vec())?;

        assert!(html.contains(OAUTH_RESULT_STORAGE_KEY));
        assert!(html.contains(OAUTH_RESULT_CHANNEL));
        assert!(html.contains(crate::legacy_identity::OAUTH_RESULT_STORAGE_KEY));
        assert!(html.contains(crate::legacy_identity::OAUTH_RESULT_CHANNEL));
        assert!(html.contains(crate::legacy_identity::OAUTH_COMPLETE_EVENT_TYPE));
        Ok(())
    }
}
