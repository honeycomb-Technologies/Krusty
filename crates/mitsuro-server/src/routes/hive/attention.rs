...
use mitsuro_core::agent::DelegatedRunStage;
use mitsuro_core::storage::{
    Database, DelegatedRunStore, HiveAttentionItemState, HiveAttentionStateStore,
    HiveRuntimeStateStatus, MessageStore, RuntimeTraceEvent, RuntimeTraceStore, SessionType,
    StoredMessageRecord,
};

use super::super::session_access::{current_user_id, load_owned_session_of_type};
use super::current::{
    build_hive_current_response, parse_timestamp, HiveCurrentResponse, HiveCurrentRunSummary,
};
...
pub(super) struct HiveAttentionResponse {
    pub(super) items: Vec<HiveAttentionItemSummary>,
    pub(super) unread_count: usize,
    pub(super) badge_count: usize,
}

#[derive(Debug, Serialize, Clone)]
pub(super) struct HiveAttentionItemSummary {
    pub(super) id: String,
...
) -> Result<Json<HiveAttentionResponse>, AppError> {
    let thread_context =
        load_attention_thread_context(&state, user.as_ref(), query.thread_session_id.as_deref())?;
    let current = build_hive_current_response(&state, user.as_ref()).await?;
...
fn maybe_keep_attention_item(
    item: HiveAttentionItemSummary,
    state_by_id: &HashMap<String, HiveAttentionItemState>,
) -> Option<HiveAttentionItemSummary> {
...
fn apply_attention_state(
    mut item: HiveAttentionItemSummary,
    state: Option<&HiveAttentionItemState>,
) -> HiveAttentionItemSummary {
...
fn build_run_action_attention_item(
    ...
    Some(HiveAttentionItemSummary {
        id: format!("run:{session_id}:{kind}"),
        kind: kind.to_string(),
        section: "needs_action".to_string(),
...
fn build_delegated_completion_attention_item(
    run: &HiveCurrentRunSummary,
    delegated_runs: &[mitsuro_core::storage::DelegatedRunRecord],
    thread_context: Option<&AttentionThreadContext>,
) -> Option<HiveAttentionItemSummary> {
...
fn run_has_scheduled_origin(run: &HiveCurrentRunSummary) -> bool {
    matches!(
        run.runtime
            .as_ref()
...
fn run_is_pending_schedule(run: &HiveCurrentRunSummary) -> bool {
    matches!(
        run.runtime.as_ref(),
        Some(runtime)
...
fn delegated_role_label(role: mitsuro_core::storage::DelegatedRunRole) -> &'static str {
    match role {
        mitsuro_core::storage::DelegatedRunRole::Explore => "Explore",
        mitsuro_core::storage::DelegatedRunRole::Build => "Build",
        mitsuro_core::storage::DelegatedRunRole::Planner => "Plan",
        mitsuro_core::storage::DelegatedRunRole::Verifier => "Verify",
    }
}

fn delegated_scope_label(scopes: &[mitsuro_core::storage::DelegatedRunScope]) -> String {
...
fn delegated_detail(record: &mitsuro_core::storage::DelegatedRunRecord) -> Option<String> {
...
fn is_failed_attention_run(run: &HiveCurrentRunSummary) -> bool {
    run.runtime
        .as_ref()
        .map(|runtime| runtime.status == HiveRuntimeStateStatus::Error)
        .unwrap_or(false)
        || run.agent_state == "error"
}

fn run_is_completed(run: &HiveCurrentRunSummary) -> bool {
    ...
    !has_open_tasks
        && !matches!(
            ...
            Some(HiveRuntimeStateStatus::Running)
                ...
                | Some(HiveRuntimeStateStatus::Paused)
                | Some(HiveRuntimeStateStatus::AwaitingInput)
        )
}
