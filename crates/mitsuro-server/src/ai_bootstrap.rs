...
use anyhow::Result;
use mitsuro_core::ai::catalog::{CatalogAuthKind, CatalogCredential};
use mitsuro_core::ai::client::{AiClient, AiClientConfig};
use mitsuro_core::ai::format_detection::detect_api_format;
...
use mitsuro_core::ai::providers::{builtin_providers, get_provider, ProviderId};
use mitsuro_core::auth::{OpenAIAuthResolution, OpenAIAuthType};
use mitsuro_core::constants;
use mitsuro_core::storage::credentials::{ActiveProviderStore, CredentialStore};
...
impl AiBootstrapPolicy {
    fn explicit_provider_from_env(self) -> Option<ProviderId> {
        mitsuro_core::identity::env_var("MITSURO_PROVIDER")
            .ok()
            .as_deref()
    ...
    fn explicit_model_from_env(self) -> Option<String> {
        mitsuro_core::identity::env_var("MITSURO_MODEL")
            .ok()
...
fn create_ai_client_for_provider_with_metadata(
    ...
    let (config, api_key) = match provider {
        ...
        ProviderId::Grok => {
            ...
            let resolved = mitsuro_core::auth::resolve_grok_auth(credentials);
...
fn resolve_openai_auth_for_model(
    ...
    let Some(scope) = auth_scope else {
        let resolved = mitsuro_core::auth::resolve_openai_auth(credentials, model);
    ...
    let identity =
        mitsuro_core::ai::catalog::credentials_for_dynamic_models(ProviderId::OpenAI, credentials)
            .into_iter()
...
pub(crate) async fn refresh_provider_model_catalog(
    ...
    if mitsuro_core::ai::catalog::credentials_for_dynamic_models(provider, &credential_snapshot)
        .is_empty()
    {
    ...
    let models =
        mitsuro_core::ai::catalog::fetch_dynamic_models_for_store(provider, &credential_snapshot)
            .await?;
    ...
    let revision = format!(
        "{:016x}",
        mitsuro_core::ai::models::model_catalog_fingerprint(&models)
    );
    registry
        .set_models_with_catalog(
            provider,
