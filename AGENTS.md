...
- Versioning, changelogs, and `v{version}` tags are owned by [Sampo](https://github.com/bruits/sampo). User-visible PRs add a changeset under `.sampo/changesets/` (`sampo add -p cargo/mitsuro -b patch -t Added -m "..."`). CI on `main` opens a Release PR, then tags after that PR merges. The existing `.github/workflows/release.yml` pipeline still builds binaries on `v*` tags and prefers Sampo changelog notes.
